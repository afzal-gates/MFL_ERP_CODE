(function () {
    'use strict';
    angular.module('multitex.purchase').controller('GeneralPOListController', ['$q', 'config', 'PurchaseDataService', '$stateParams', '$state', '$scope', GeneralPOListController]);
    function GeneralPOListController($q, config, PurchaseDataService, $stateParams, $state, $scope) {
        ...
        function getSupplierList() {
            PurchaseDataService.getDataByFullUrl('/api/knit/KnitYarnReq/GetPOList?pIS_LOCAL_CASH_P=Y').then(function (res) {
        ...
        vm.gridOptions = {
            ...
            columns: [
...

                {
                    ...
                    title: "Action",
                    template: '<a ui-sref="GeneralPO({pCM_IMP_PO_H_ID:dataItem.CM_IMP_PO_H_ID,pREVISE:0})" class="btn btn-xs blue"><i class="fa fa-edit"> {{dataItem.NXT_ACTION_NAME}}</i></a>' +
                        ' <a ui-sref="GeneralPO({pCM_IMP_PO_H_ID:dataItem.CM_IMP_PO_H_ID,pREVISE:1})" class="btn btn-xs yellow-gold"><i class="fa fa-edit"> Amendment</i></a></a>',
                    width: "10%"
