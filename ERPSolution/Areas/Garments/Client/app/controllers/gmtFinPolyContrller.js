...
(function () {
    ...
    angular.module('multitex.garments').controller('GmtFinPolyController', ['logger', 'config', '$q', '$scope', '$http', 'exception', '$filter', '$state', '$stateParams', '$localStorage', 'FloorData', 'GarmentsDataService', 'Dialog', '$modal', 'cur_date_server', GmtFinPolyController]);
    function GmtFinPolyController(logger, config, $q, $scope, $http, exception, $filter, $state, $stateParams, $localStorage, FloorData, GarmentsDataService, Dialog, $modal, cur_date_server) {
        ...
        if ($localStorage.GMT_FIN_PLY_CALENDAR_DT) {
            $scope.dt = $filter('date')($localStorage.GMT_FIN_PLY_CALENDAR_DT, vm.dtFormat);
            vm.selectedDt = $filter('date')($localStorage.GMT_FIN_PLY_CALENDAR_DT, vm.dtFormat);
        }
        else {
        ...
        if (!$localStorage.GMT_FIN_PLY_HR_NO) {
            vm.selectedHrNo = 1;
        }
        else {
            vm.selectedHrNo = $localStorage.GMT_FIN_PLY_HR_NO;
        }


        activate();
        function activate() {
            var promise = [getGmtFinPolyData(vm.selectedDt, vm.selectedFlr), getByrAccWiseStyleExtList()];
        ...
        vm.onDateSelect = function (dt) {
            ...
            //$localStorage.GMT_FIN_PLY_CALENDAR_DT = vm.selectedDt;

            getGmtFinPolyData(vm.selectedDt, vm.selectedFlr);
        };

        vm.OnChangeFloor = function (itm) {
            getGmtFinPolyData(vm.selectedDt, itm.HR_PROD_FLR_ID);

            vm.selectedFlr = itm.HR_PROD_FLR_ID;
            //$localStorage.GMT_FIN_PLY_PROD_FLR_LST = itm.HR_PROD_FLR_ID;
        ...
        vm.onFocus = function (hrItm) {
            ...
            var obj=_.maxBy(vm.GmtFinPolyData, function (o) { return o.HR_NO; });
        ...
        function getGmtFinPolyData(pCALENDAR_DT, pHR_PROD_FLR_LST) {
            return GarmentsDataService.getDataByFullUrl('/api/gmt/GmtFin/getGmtFinPolyData?pCALENDAR_DT=' + pCALENDAR_DT + '&pHR_PROD_FLR_LST=' + pHR_PROD_FLR_LST).then(function (res) {
                vm.GmtFinPolyData = res;
            });
        }
        ...
        vm.ordItmColorGenerate = function (hrItm) {
            ...
            return GarmentsDataService.getDataByFullUrl('/api/gmt/GmtFin/GetGmtFinPolyOrderList?pMC_ORDER_H_LST=' + vOrdLst).then(function (res) {
        ...
        vm.save = function () {
            ...
            $localStorage.GMT_FIN_PLY_PROD_FLR_LST = vm.selectedFlr;
            $localStorage.GMT_FIN_PLY_CALENDAR_DT = vm.selectedDt;
            $localStorage.GMT_FIN_PLY_HR_NO = vm.selectedHrNo;
            ...
            angular.forEach(vm.GmtFinPolyData, function (val, key) {
                ...
                angular.forEach(val['itemsOrder'], function (val1, key1) {
                    ...
                    angular.forEach(val1['itemsOrdItmColor'], function (val2, key2) {
                        ...
                        angular.forEach(val2['itemsOrdSize'], function (val3, key3) {
                            gmtFinPolyDtl.push({
                                HDR_ID: vHdrIdx, GMT_FIN_POLY_D_ID: val3.GMT_FIN_POLY_D_ID, GMT_FIN_POLY_H_ID: val3.GMT_FIN_POLY_H_ID, MC_ORDER_SIZE_ID: val3.MC_ORDER_SIZE_ID,
                                ...
                                MC_ORDER_SHIP_ID: val3.MC_ORDER_SHIP_ID, POLY_QTY: val3.POLY_QTY
                            });                            
                        });
                ...
                gmtFinPolyHdr.push({
                    HDR_ID: vHdrIdx, GMT_FIN_POLY_H_ID: val.GMT_FIN_POLY_H_ID, HR_PROD_FLR_ID: val.HR_PROD_FLR_ID, GMT_PROD_PLN_CLNDR_ID: val.GMT_PROD_PLN_CLNDR_ID,
            ...
            submitData['GMT_FIN_POLY_H_XML'] = GarmentsDataService.xmlStringShort(gmtFinPolyHdr.map(function (ob) {
                return ob;
            }));

            submitData['GMT_FIN_POLY_D_XML'] = GarmentsDataService.xmlStringShort(gmtFinPolyDtl.map(function (ob) {
                return ob;
            }));
            ...
            Dialog.confirm('Do you want to save?', 'Confirmation', ['Yes', 'No'])
                 .then(function () {
                     ...
                     $http({
                         ...
                         url: '/api/gmt/GmtFin/BatchSaveGmtFinPoly',
                         method: 'post',
                         data: submitData
                     }).then(function (res) {
                         ...
                         if (res.data.success === false) {
                             ...
                             if (res.data.PMSG.substr(0, 9) == 'MULTI-001') {                                 
                                 $state.go('GmtFinPoly', { pCALENDAR_DT: vm.selectedDt, pHR_PROD_FLR_LST: vm.selectedFlr }, { reload: true });
                             }
