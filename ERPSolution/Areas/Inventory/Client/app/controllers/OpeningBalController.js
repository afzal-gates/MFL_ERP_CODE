...
(function () {
    'use strict';
    angular.module('multitex.knitting').controller('OpeningBalController', ['$q', 'config', 'InventoryDataService', '$stateParams', '$state', '$scope', 'commonDataService', 'Dialog', '$filter', '$timeout', OpeningBalController]);
    function OpeningBalController($q, config, InventoryDataService, $stateParams, $state, $scope, commonDataService, Dialog, $filter, $timeout) {
        ...
        vm.getStoreByID = function () {
            ...
            if (vm.form.HR_COMPANY_ID > 0) {
                vm.isExistsOpnBal = 'N';
                InventoryDataService.getDataByFullUrl('/api/common/StoreListByOfcComCatID?pHR_COMPANY_ID=' + (vm.form.HR_COMPANY_ID || 0)).then(function (res) {
        ...
        vm.TotalPriceAmount = function () {
            if (vm.form.OB_RCV_QTY > 0 && vm.form.UNIT_PRICE > 0)
                vm.form.COST_PRICE = vm.form.OB_RCV_QTY * vm.form.UNIT_PRICE;
            else
        ...
        //            transport: {
        //                read: function (e) {
        //                    InventoryDataService.getDataByFullUrl('/api/common/GetStoreInfo?pINV_ITEM_CAT_LST=128').then(function (res) {
            ...
            vm.rcvItemGridDataSource = new kendo.data.DataSource({
                ...
                transport: {
                    read: function (e) {
                        ...
                        var url = '/api/inv/StoreReceive/GetOpnBalList';
                        url += '?pageNumber=1&pageSize=1&pHR_COMPANY_ID=' + (vm.form.HR_COMPANY_ID || 0) + '&pSCM_STORE_ID=' + (vm.form.SCM_STORE_ID || 0);
                        ...
                        InventoryDataService.getDataByFullUrl(url).then(function (res) {
                            ...
                            if (res.length > 0) {
                                vm.isExistsOpnBal = 'Y';
                                vm.form.OB_RCV_DT = res[0]['OB_RCV_DT'];
                            }
                            e.success(res);
                        });
                    }
        ...
        vm.gridOptionsItem = {
            ...
            columns: [
                ...
                { field: "OB_RCV_QTY", title: "Opening Qty.", width: "10%" },
                { field: "QTY_MOU_CODE", title: "UoM", type: "string", width: "10%" },
                { field: "UNIT_PRICE", title: "Unit Price", type: "decimal", template: "#= kendo.toString(UNIT_PRICE,'n') #", width: "10%" },
                { field: "COST_PRICE", title: "Cost Price", type: "decimal", template: "#= kendo.toString(COST_PRICE,'n') #", width: "10%" },
                {
                    title: "Action",
        ...
        function ItemList() {
            vm.itemOptions = {
                ...
                select: function (e) {
                    var item = this.dataItem(e.item);
                    console.log(item);
                    vm.form.UNIT_PRICE = item.PURCH_PRICE;
                    vm.form.COST_PRICE = item.COST_PRICE;
        ...
        vm.editItemData = function (data) {
            ...
            vm.form.SCM_GEN_ITEM_OB_ID = itmData.SCM_GEN_ITEM_OB_ID;
        ...
        vm.removeItemData = function (dataOri) {
            ...
            Dialog.confirm('Removing "' + dataOri.ITEM_NAME_EN + '.', 'Attention', ['Yes', 'No'])
                 .then(function () {
                     ...
                     return InventoryDataService.saveDataByUrl(data, '/StoreReceive/Delete').then(function (res) {
            ...
            Dialog.confirm('Finalizing "' + dataOri.ITEM_NAME_EN + '.', 'Attention', ['Yes', 'No'])
                 .then(function () {
                     ...
                     return InventoryDataService.saveDataByUrl(data, '/StoreReceive/Save').then(function (res) {
                         ...
                         vm.form = { HR_COMPANY_ID: data.HR_COMPANY_ID, SCM_STORE_ID: data.SCM_STORE_ID, OB_RCV_DT: data.OB_RCV_DT, INV_ITEM_CAT_ID: data.INV_ITEM_CAT_ID, INV_ITEM_ID: '', QTY_MOU_ID: 1 };
        ...
        vm.reset = function () {
            ...
            vm.form = { HR_COMPANY_ID: data.HR_COMPANY_ID, SCM_STORE_ID: data.SCM_STORE_ID, OB_RCV_DT: data.OB_RCV_DT, RF_CURRENCY_ID: data.RF_CURRENCY_ID, INV_ITEM_CAT_ID: data.INV_ITEM_CAT_ID, INV_ITEM_ID: '', QTY_MOU_ID: 1 };
        };
        ...
        $scope.$watchGroup(['vm.form.HR_COMPANY_ID', 'vm.form.SCM_STORE_ID'], function (newVal, oldVal) {
        ...
        vm.selectAll = function (v, index) {
            var data = vm.rcvItemGridDataSource.data();
            console.log(data);
        ...
        vm.submitAll = function (dataOri) {
            ...
            if (fnValidate() == true) {
                ...
                return InventoryDataService.saveDataByUrl(data, '/StoreReceive/SaveOB').then(function (res) {
        ...
        vm.finalizeAllData = function () {
            ...
            Dialog.confirm(vMsg, 'Confirmation', ['Yes', 'No']).then(function () {
                ...
                submitRcvData.MCN_NDL_OPN_BAL_XML = InventoryDataService.xmlStringShort(rcvData.map(function (ob) {
                    return ob;
                }));

                console.log(submitRcvData);

                return InventoryDataService.saveDataByUrl(submitRcvData, '/StoreReceive/FinalizeAll').then(function (res) {
