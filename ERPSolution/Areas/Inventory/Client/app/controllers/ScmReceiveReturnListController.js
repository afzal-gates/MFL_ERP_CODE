...
(function () {
    'use strict';
    angular.module('multitex.inventory').controller('ScmReceiveReturnListController', ['$q', 'config', 'InventoryDataService', '$stateParams', '$state', '$scope', '$filter', ScmReceiveReturnListController]);
    function ScmReceiveReturnListController($q, config, InventoryDataService, $stateParams, $state, $scope, $filter) {
        ...
        function dailyReceiveList() {
            return vm.gridOptionsDS = new kendo.data.DataSource({
                ...
                schema: {
                    ...
                    model: {
                        fields: {
                            SCM_STR_RET_H_ID: { type: "string" },
                            RET_CHALAN_NO: { type: "string" },
        ...
        vm.gridOptions = {
            ...
            sortable: true,
            columns: [
                { field: "RET_CHALAN_NO", title: "MRR No", type: "string", width: "10%" },
                { field: "RET_CHALAN_DT", title: "MRR Date", type: "date", template: "#= kendo.toString(kendo.parseDate(RET_CHALAN_DT,'yyyy-MM-dd'),'dd-MMM-yyyy') #", width: "10%" },
                ...
                { field: "REMARKS", title: "Remarks", type: "string", width: "10%" },
                {
                    title: "Action",
                    template: function () {
                        return '</a><a ui-sref="ScmReceiveReturn({pSCM_STR_RET_H_ID:dataItem.SCM_STR_RET_H_ID})" class="btn btn-xs green"><i class="fa fa-eye"> View</i></a> <a ui-sref="ScmReceiveReturn({pSCM_STR_RET_H_ID:dataItem.SCM_STR_RET_H_ID})"  ng-if="dataItem.IS_FINALIZED==' + "'N'" + '" class="btn btn-xs blue"><i class="fa fa-edit"> Edit</i></a> <a ng-click="vm.printMRR(dataItem);" class="btn btn-xs green"><i class="fa fa-print"> Print</i></a></a>';
                    },
                    width: "10%"
                }
            ]
