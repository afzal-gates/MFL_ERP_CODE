(function () {
    'use strict';
    angular.module('multitex.dyeing').controller('DyeChemicalStoreTransferController', ['$q', 'config', 'DyeingDataService', '$stateParams', '$state', '$scope', 'commonDataService', 'formData', 'cur_user_id', '$filter', DyeChemicalStoreTransferController]);
    function DyeChemicalStoreTransferController($q, config, DyeingDataService, $stateParams, $state, $scope, commonDataService, formData, cur_user_id, $filter) {
        ...
        vm.form = formData.DYE_STR_TR_REQ_H_ID ? formData : { RF_REQ_TYPE_ID: '12', HR_COMPANY_ID: '0', INV_ITEM_CAT_ID: '2', STR_REQ_BY: cur_user_id, FRM_STORE_ID: 0, TO_STORE_ID: 0, RF_ACTN_VIEW: 0, HR_DEPARTMENT_ID: 30 };
        ...
        vm.submitAll = function (dataOri) {
            ...
            if (fnValidate() == true) {
                ...
                data["XML_REQ_D"] = DyeingDataService.xmlStringShort(vm.ReceiveItemList.data().map(function (o) {
                    return {
                        DYE_STR_TR_REQ_D_ID: o.DYE_STR_TR_REQ_D_ID == null ? 0 : o.DYE_STR_TR_REQ_D_ID,
                        DYE_STR_TR_REQ_H_ID: o.DYE_STR_TR_REQ_H_ID == null ? 0 : o.DYE_STR_TR_REQ_H_ID,
                ...
                var id = vm.form.DYE_STR_TR_REQ_H_ID


                var _invdate = $filter('date')(data.STR_TR_REQ_DT, 'yyyy-MM-ddTHH:mm:ss');
                data["STR_TR_REQ_DT"] = _invdate;

                return DyeingDataService.saveDataByUrl(data, '/DyeChemicalStoreTransfer/Save').then(function (res) {
                    ...
                    if (res.success === false) {
                        ...
                        if (id > 0) {
                            ...
                            $state.go($state.current, { pDYE_STR_TR_REQ_H_ID: res.data.OPDYE_STR_TR_REQ_H_ID }, { reload: true });
                        }
                        else {
                            config.appToastMsg(res.data.PMSG);
                            if (parseInt(res.data.OPDYE_STR_TR_REQ_H_ID) > 0) {
                                vm.form.DYE_STR_TR_REQ_H_ID = res.data.OPDYE_STR_TR_REQ_H_ID;
                                $state.go($state.current, { pDYE_STR_TR_REQ_H_ID: res.data.OPDYE_STR_TR_REQ_H_ID }, { reload: true });
                            }
                        }
        ...
        vm.updateAll = function (dataOri) {
            ...
            if (fnValidate() == true) {
                ...
                return DyeingDataService.saveDataByUrl(data, '/DyeChemicalStoreTransfer/Update').then(function (res) {
