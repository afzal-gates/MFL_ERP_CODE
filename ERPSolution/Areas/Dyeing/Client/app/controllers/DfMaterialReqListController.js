(function () {
    'use strict';
    angular.module('multitex.dyeing').controller('DfMaterialReqListController', ['$q', 'config', 'DyeingDataService', '$stateParams', '$state', '$rootScope', '$scope', 'Hub', 'cur_user_id', '$filter', 'Dialog', DfMaterialReqListController]);
    function DfMaterialReqListController($q, config, DyeingDataService, $stateParams, $state, $rootScope, $scope, Hub, cur_user_id, $filter, Dialog) {
            ...
            return vm.gridOptionsDS = new kendo.data.DataSource({
                ...
                transport: {
                    read: function (e) {
                        ...
                        DyeingDataService.getDataByFullUrl('/api/dye/DCIssueRequisition/SelectAll/' + params.page + '/' + params.pageSize + '?' + pm + '&pRF_REQ_TYPE_ID=9' + '&pUSER_ID=' + cur_user_id).then(function (res) {
                            e.success(res);
                        });
                    }
                },
        ...
        vm.gridOptions = {
            ...
            columns: [
...

                {
                    title: "Action",
                    template: function () {
                        return '</a><a ui-sref="DfMaterialReq({pDYE_STR_REQ_H_ID:dataItem.DYE_STR_REQ_H_ID})" ng-if="(dataItem.ACTN_ROLE_FLAG==' + "'RI'" + ' || dataItem.ACTN_ROLE_FLAG==' + "'RA'" + ') && dataItem.PERMISSION==1" class="btn btn-xs blue"><i class="fa fa-edit"> {{dataItem.NXT_ACTION_NAME}}</i></a>' +
                            ...
                            ' <a ui-sref="DfMaterialReq({pDYE_STR_REQ_H_ID:dataItem.DYE_STR_REQ_H_ID})" ng-if="dataItem.ACTN_ROLE_FLAG==' + "'DN'" + ' && (dataItem.LOAD_TIME>=' + "'2017-01-01'" + ' || dataItem.UN_LOAD_TIME>=' + "'2017-01-01'" + ')" class="btn btn-xs blue"><i class="fa fa-edit"> Complete</i></a>' +
