(function () {
    'use strict';
    angular.module('multitex.dyeing').controller('DCIssueAgainstLoanRequisitionController', ['$q', 'config', 'DyeingDataService', '$stateParams', '$state', '$scope', 'commonDataService', 'formData', 'cur_user_id', DCIssueAgainstLoanRequisitionController]);
    function DCIssueAgainstLoanRequisitionController($q, config, DyeingDataService, $stateParams, $state, $scope, commonDataService, formData, cur_user_id) {
        ...
        vm.copyAllRequiredQty = function () {
            for (var i = 0; i < vm.ReceiveItemList.length; i++)
                if (parseFloat(vm.ReceiveItemList[i].SUB_STK) >= parseFloat(vm.ReceiveItemList[i].RQD_QTY))
                    vm.ReceiveItemList[i].ISS_QTY = vm.ReceiveItemList[i].RQD_QTY;
                else
                ...
                return DyeingDataService.saveDataByUrl(data, '/DCIssue/Save').then(function (res) {
                    ...
                    if (res.success === false) {
                        ...
                        if (id > 0) {
                            ...
                            $state.go("DCIssueAgainstLoanRequisition", { pDYE_DC_ISS_H_ID: res.data.OPDYE_DC_ISS_H_ID }, { reload: true });
                        }
                        else {
                            config.appToastMsg(res.data.PMSG);
                            if (parseInt(res.data.OPDYE_DC_ISS_H_ID) > 0) {
                                //alert(res.data.OPDYE_DC_ISS_H_ID);
