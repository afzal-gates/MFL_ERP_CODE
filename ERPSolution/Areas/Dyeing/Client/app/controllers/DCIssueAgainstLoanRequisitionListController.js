(function () {
    'use strict';
    angular.module('multitex.dyeing').controller('DCIssueAgainstLoanRequisitionListController', ['$q', 'config', 'DyeingDataService', '$stateParams', '$state', '$rootScope', '$scope', 'Hub', 'cur_user_id', DCIssueAgainstLoanRequisitionListController]);
    function DCIssueAgainstLoanRequisitionListController($q, config, DyeingDataService, $stateParams, $state, $rootScope, $scope, Hub, cur_user_id) {
        var vm = this;
            ...
            return vm.gridOptionsDS = new kendo.data.DataSource({
                ...
                transport: {
                    read: function (e) {
                        ...
                        DyeingDataService.getDataByFullUrl('/api/dye/DCIssueRequisition/SelectAll/' + params.page + '/' + params.pageSize + '?' + pm + '&pRF_REQ_TYPE_ID=11&pUSER_ID=' + cur_user_id + '&pOption=3006').then(function (res) {
                            e.success(res);
                        });
                    }
                },
        ...
        vm.gridOptions = {
            ...
            columns: [
...

                {
                    title: "Action",
                    template: function () {
                        return '</a><a ui-sref="DCIssueRequisition({pDYE_STR_REQ_H_ID:dataItem.DYE_STR_REQ_H_ID})" ng-if="(dataItem.ACTN_ROLE_FLAG==' + "'RI'" + ' || dataItem.ACTN_ROLE_FLAG==' + "'RA'" + ') && dataItem.PERMISSION==1" class="btn btn-xs blue"><i class="fa fa-edit"> {{dataItem.NXT_ACTION_NAME}}</i></a> ' +
                            '<a ui-sref="DCIssueAgainstLoanRequisition({pDYE_STR_REQ_H_ID:dataItem.DYE_STR_REQ_H_ID})" ng-if="dataItem.ACTN_ROLE_FLAG==' + "'SI'" + ' && dataItem.PERMISSION==1" class="btn btn-xs blue"><i class="fa fa-edit"> {{dataItem.NXT_ACTION_NAME}}</i></a></a>';
                    },
                    //template: function () {
                    //    return "</a><a ui-sref='DCIssueRequisition({pDYE_STR_REQ_H_ID:dataItem.DYE_STR_REQ_H_ID})' ng-if='dataItem.RF_ACTN_STATUS_ID<=4 && dataItem.PERMISSION==1' class='btn btn-xs blue'><i class='fa fa-edit'> {{dataItem.NXT_ACTION_NAME}}</i></a> " +
                    //        "<a ui-sref='DCIssueAgainstLoanRequisition({pDYE_STR_REQ_H_ID:dataItem.DYE_STR_REQ_H_ID})' ng-if='dataItem.RF_ACTN_STATUS_ID==5 && dataItem.PERMISSION==1' class='btn btn-xs blue'><i class='fa fa-edit'> {{dataItem.NXT_ACTION_NAME}}</i></a></a>";
                    //},
                    width: "10%"
                }
            ]
            ...
            return {
                ...
                columns: [
               {
                ...
                },
                {
                    ...
                    template: function () {
                        return "</a><a ui-sref='DCIssueAgainstLoanRequisition({pDYE_DC_ISS_H_ID:dataItem.DYE_DC_ISS_H_ID})' ng-if='!dataItem.LAST_UPDATED_BY>0 && dataItem.PERMISSION==1' class='btn btn-xs blue'><i class='fa fa-edit'> Edit</i></a>";
                    },
                    width: "10%"
                },
