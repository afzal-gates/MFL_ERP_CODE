(function () {
    'use strict';
    angular.module('multitex.dyeing').controller('DCBatchProgramRequisitionController', ['$q', 'config', 'DyeingDataService', '$stateParams', '$state', '$scope', 'commonDataService', 'formData', 'cur_user_id', 'Dialog', '$filter', '$modal', DCBatchProgramRequisitionController]);
    function DCBatchProgramRequisitionController($q, config, DyeingDataService, $stateParams, $state, $scope, commonDataService, formData, cur_user_id, Dialog, $filter, $modal) {
        ...
        function activate() {
            var promise = [GetBatchList(), getdosingTemplateList(), GetItemCategoryList(), GetChemicalCategoryList(), getItemDataByCategory(), GetMachineList(),
                ReceiveItemList(), GetStatusList(), GetCompanyList(), GetReqSourceList()];
        ...
        vm.openLabdipUI = function () {

            var modalInstance = $modal.open({
                animation: true,
                templateUrl: '/Dyeing/Dye/_LabdipRecipeModal',
                controller: 'LabdipRecipeModalController',
                controllerAs: 'vm',
                size: 'lg',
                windowClass: 'app-modal-window',
                resolve: {
                    formData: function (DyeingDataService, $stateParams) {
                        var lst = { MC_BYR_ACC_ID: 1, MC_COLOR_ID: vm.form.MC_COLOR_ID, MC_STYLE_H_ID: vm.form.MC_STYLE_H_ID, MC_BYR_ACC_GRP_ID: vm.form.MC_BYR_ACC_GRP_ID }
                        return lst;
                    }
                }
            });
        ...
        vm.onChangeTemplate = function (e) {
            ...
            vm.recipeItemList = new kendo.data.DataSource({
                transport: {
                    read: function (e) {
                        DyeingDataService.getDataByFullUrl('/api/dye/LabdipRecipe/GetLDDosingTempleteDtl?pDYE_DOSE_TMPLT_H_ID=' + item.DYE_DOSE_TMPLT_H_ID + '&pSCM_STORE_ID=' + vm.form.SCM_STORE_ID).then(function (res) {
        ...
        vm.loadMachineByCompany = function () {
            DyeingDataService.getDataByFullUrl('/api/Dye/DyeMachine/GetMachineListByCompany?pHR_COMPANY_ID=' + vm.form.HR_COMPANY_ID).then(function (res) {
                vm.machineList = res;

                if (vm.form.HR_COMPANY_ID > 0) {
                    var sList = _.filter(vm.storeList.data(), function (x) { return x.HR_COMPANY_ID == vm.form.HR_COMPANY_ID });
                    if (sList.length > 0)
                        vm.form.SCM_STORE_ID = sList[0].SCM_STORE_ID;
                }
            });
        ...
        vm.printBatchCard = function (dataItem) {
            ...
            //vm.form.IS_EXCEL_FORMAT = (isExcel || 'N');
        ...
        vm.printBatchCardDraft = function (dataItem) {

            vm.form.REPORT_CODE = 'RPT-4101';
        ...
        vm.getBatchDetails = function (e) {
            ...
            if (obj.DYE_BT_CARD_H_ID > 0) {
                if (fnValidateSub2() == true) {
                    ...
                    if (count > 0 && obj.DYE_BATCH_NO.includes('S-') == false) {
        ...
        function GetBatchList() {
            ...
            if ($stateParams.pDYE_STR_REQ_H_ID > 0) {
                ...
                if ($stateParams.pLOT_ID) {
                    DyeingDataService.getDataByFullUrl('/api/Dye/DCBatchProgramRequisition/GetBatchFabrication?pDYE_BATCH_NO=' + ($stateParams.pLOT_ID || "") + '&pOption=3007').then(function (res) {

                        vm.BatchFabricList = res.fab;
                    });
                }
                    //else if ($stateParams.pON_LINE_QC_ID) {
                    //    DyeingDataService.getDataByFullUrl('/api/Dye/DCBatchProgramRequisition/GetBatchFabrication?pDYE_BATCH_NO=' + ($stateParams.pLOT_ID || "") + '&pOption=3007').then(function (res) {
                    ...
                    DyeingDataService.getDataByFullUrl('/api/Dye/DCBatchProgramRequisition/GetBatchFabrication?pDYE_BATCH_NO=' + (formData[0].DYE_BATCH_NO || "") + '&pOption=3005').then(function (res) {
                ...
                DyeingDataService.getDataByFullUrl('/api/Dye/DCBatchProgramRequisition/' + OptUrl + '?pDYE_STR_REQ_H_ID=' + ($stateParams.pDYE_STR_REQ_H_ID || 0) + '&pSCM_STORE_ID=' + vm.form.SCM_STORE_ID).then(function (output) {
            ...
            if (fnValidateSub() == true) {
                ...
                if (vm.formItem.uid) {
                    ...
                    if (count <= 1) {
                        ...
                        row["DYE_LOT_NO"] = vm.formItem.DYE_LOT_NO;
                        ...
                        for (var x = 0; x < vm.BatchProgramList.data().length; x++) {
                            ...
                            if (vm.BatchProgramList.data()[x].ACT_BATCH_QTY > 0)
                                sum = sum + vm.BatchProgramList.data()[x].ACT_BATCH_QTY;
                            else
                                sum = sum + vm.formItem.ACT_BATCH_QTY;
                        }
                    ...
                    DyeingDataService.getDataByFullUrl('/api/dye/LabdipRecipe/GetLDDosingTempleteDtl?pDYE_DOSE_TMPLT_H_ID=' + (vm.form.DYE_DOSE_TMPLT_H_ID || 0) + '&pMC_LD_RECIPE_H_ID=' + (vm.form.MC_LD_RECIPE_H_ID || 0) + "&pOption=" + pOption + '&pSCM_STORE_ID=' + vm.form.SCM_STORE_ID).then(function (output) {
                        var list = _.uniqBy(output, function (d) { return d.PRD_PHASE_NAME });
                        //console.log(output);
                        //console.log(list);
                        vm.recipeGroupList = list;
                    ...
                    for (var i = 0; i < item.length; i++) {
                        ...
                        if (count == 0) {
                            ...
                            if (item[i].ACT_BATCH_QTY == 0)
                                item[i].ACT_BATCH_QTY = parseFloat(vm.formItem.ACT_BATCH_QTY);
                            ...
                            for (var x = 0; x < vm.BatchProgramList.data().length; x++)
                                if (vm.BatchProgramList.data()[x].ACT_BATCH_QTY > 0)
                                    ...
                                    sum = sum + parseFloat(vm.formItem.ACT_BATCH_QTY);
                            vm.form.TOTAL_WEIGHT = sum;
                            vm.form.TOTAL_WATER = sum * vm.form.LQR_RATIO;
        ...
        vm.gridOptionsBatch = {
            ...
            columns: [
                ...
                { field: "DF_BT_SUB_LOT_ID", hidden: true },

                { field: "SL_NO", title: "SL No", type: "string", width: "10%", template: "<span class='row-number'></span>" },

                { field: "DYE_BATCH_NO", title: "Batch No", type: "string", width: "30%", },
                { field: "DYE_LOT_NO", title: "Lot No", type: "string", width: "18%", },
                {
                    field: "ACT_BATCH_QTY", title: "Qty",
                    headerAttributes: { style: "text-align:right" },
                    attributes: { class: "text-right" },
                    format: "{0:n}", type: "number", width: "12%"
                },
        ...
        vm.PreItemCalc = function (pItem, col) {
            ...
            if (pItem.INV_ITEM_ID > 0)
                DyeingDataService.getDataByFullUrl('/api/dye/DCBatchProgramRequisition/GetStockInfoForBP/' + pItem.INV_ITEM_ID + '/' + (vm.form.SCM_STORE_ID || 0)).then(function (res) {
                    pItem.CENTRAL_STK = res.CENTRAL_STK;
                    pItem.SUB_STK = res.SUB_STK;
                });
        }
        ...
        vm.saveAll = function (dataOri) {

            if (fnValidate() == true) {
                vm.showSplash = true;
                var data = angular.copy(dataOri);

                var count = _.filter(vm.recipeItemList, function (x) { return x.RQD_QTY > 0 && x.IS_FINALIZED === 'N' });
                if (count.length == 0) {
                ...
                data["XML_REQ_D1"] = DyeingDataService.xmlStringShort(vm.BatchProgramList.data().map(function (o) {
                    return {
                        ...
                        DF_BT_SUB_LOT_ID: o.DF_BT_SUB_LOT_ID == 0 ? null : o.DF_BT_SUB_LOT_ID,
                ...
                data["XML_REQ_D2"] = DyeingDataService.xmlStringShort((_.filter(vm.recipeItemList, function (x) { return x.RQD_QTY > 0 })).map(function (o) {
                    return {
                        ...
                        QTY_MOU_ID: o.QTY_MOU_ID == null ? 3 : o.QTY_MOU_ID,
                        IS_FINALIZED: o.IS_WAIT == "X" ? null : "W"
                    }
                }));


                return DyeingDataService.saveDataByUrl(data, '/DCBatchProgramRequisition/Save').then(function (res) {
        ...
        vm.submitAll = function (dataOri) {
            ...
            if (fnValidate() == true) {
                ...
                if ($stateParams.pADDITION_FLAG > 0) {
                    ...
                    if (!$stateParams.pRIB_ID)
                        data["DF_RIB_SHADE_RPT_H_ID"] = $stateParams.pRIB_ID;
                }
                ...
                data["XML_REQ_D2"] = DyeingDataService.xmlStringShort((_.filter(vm.recipeItemList, function (x) { return x.RQD_QTY > 0 })).map(function (o) {
                    return {
                        ...
                        DOSE_QTY: o.DOSE_QTY == null ? 0 : o.DOSE_QTY,
                        DOSE_MOU_ID: o.MOU_CODE == "%" ? 27 : 28,
                        STEP_NO: o.STEP_NO, // vm.recipeItemList.indexOf(o) + 1,// ++ix,
                        //STEP_NO: o.STEP_NO == null ? 0 : o.STEP_NO,
                        RQD_QTY_K: o.RQD_QTY_K == null ? 0 : o.RQD_QTY_K,
                        RQD_QTY_G: o.RQD_QTY_G == null ? 0 : o.RQD_QTY_G,
                        RQD_QTY_M: o.RQD_QTY_M == null ? 0 : o.RQD_QTY_M,
        ...
        vm.updateAll = function (dataOri) {
            ...
            if (fnValidate() == true) {
                ...
                //console.log(list);
                var ix = 0;
                data["XML_REQ_D2"] = DyeingDataService.xmlStringShort((_.filter(vm.recipeItemList, function (x) { return x.RQD_QTY > 0 && (parseFloat(x.SUB_STK)) >= parseFloat(x.RQD_QTY) })).map(function (o) {
                    return {
                        DYE_PRD_PHASE_TYPE_ID: o.DYE_PRD_PHASE_TYPE_ID == null ? 2 : o.DYE_PRD_PHASE_TYPE_ID,
                        DYE_BT_DC_REQ_D2_ID: o.DYE_BT_DC_REQ_D2_ID == null ? 0 : o.DYE_BT_DC_REQ_D2_ID,
                        DC_AGENT_ID: o.DC_AGENT_ID,
                ...
                return DyeingDataService.saveDataByUrl(data, '/DCBatchProgramRequisition/Update').then(function (res) {
