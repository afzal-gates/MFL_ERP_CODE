(function () {
    'use strict';
    angular.module('multitex.dyeing').controller('DFReceiveFinishFabricListController', ['$q', 'config', 'DyeingDataService', '$stateParams', '$state', '$rootScope', '$scope', 'Hub', 'cur_user_id', '$filter', DFReceiveFinishFabricListController]);
    function DFReceiveFinishFabricListController($q, config, DyeingDataService, $stateParams, $state, $rootScope, $scope, Hub, cur_user_id, $filter) {
            ...
            return vm.gridOptionsDS = new kendo.data.DataSource({
                ...
                transport: {
                    read: function (e) {
                        ...
                        DyeingDataService.getDataByFullUrl('/api/Dye/DfRcvFinFabric/SelectRcvAll/' + params.page + '/' + params.pageSize + '?' + pm + '&pUSER_ID=' + cur_user_id).then(function (res) {
                            e.success(res);
                        });
                    }
                },
        ...
        vm.gridOptions = {
            ...
            columns: [
                ...
                { field: "GATE_PASS_NO", title: "Gate Pass No", type: "string", width: "10%" },
                {
                    title: "Action",
                    template: function () {
                        return '</a><a ui-sref="ReceiveFinishFabric({pDF_SCO_FSTR_RCV_H_ID:dataItem.DF_SCO_FSTR_RCV_H_ID})" ng-if="dataItem.IS_FINALIZED==' + "'N'" + '" class="btn btn-xs green"><i class="fa fa-edit"> Edit</i></a> ' +
