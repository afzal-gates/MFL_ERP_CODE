...
(function () {
    'use strict';
    angular.module('multitex.dyeing').controller('ScPreTreatmentBillListController', ['$q', 'config', 'DyeingDataService', '$stateParams', '$state', '$rootScope', '$scope', 'Hub', 'cur_user_id', '$filter', ScPreTreatmentBillListController]);
    function ScPreTreatmentBillListController($q, config, DyeingDataService, $stateParams, $state, $rootScope, $scope, Hub, cur_user_id, $filter) {
        ...
        vm.sciBillGridDataSource = new kendo.data.DataSource({
            ...
            transport: {
                read: function (e) {
                    ...
                    var url = '/api/dye/DfScPtBill/GetBillList?pSCM_SUPPLIER_ID=' + (vm.form.SCM_SUPPLIER_ID || $stateParams.pSCM_SUPPLIER_ID || 0);
        ...
        vm.editBill = function (dataItem) {
            $state.go('ScPreTreatmentBill', { pDF_SCO_PT_BILL_H_ID: dataItem.DF_SCO_PT_BILL_H_ID });
        };
