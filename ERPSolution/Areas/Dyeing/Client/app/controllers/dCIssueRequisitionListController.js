(function () {
    'use strict';
    angular.module('multitex.dyeing').controller('DCIssueRequisitionListController', ['$q', 'config', 'DyeingDataService', '$stateParams', '$state', '$rootScope', '$scope', 'Hub', 'cur_user_id', '$filter', 'Dialog', DCIssueRequisitionListController]);
    function DCIssueRequisitionListController($q, config, DyeingDataService, $stateParams, $state, $rootScope, $scope, Hub, cur_user_id, $filter, Dialog) {
            ...
            return vm.gridOptionsDS = new kendo.data.DataSource({
                ...
                transport: {
                    read: function (e) {
                        ...
                        var params = webapi.parameterMap(e.data);
                        var pm = DyeingDataService.kFilterStr2QueryParam(params.filter);
                        DyeingDataService.getDataByFullUrl('/api/dye/DCIssueRequisition/SelectAll/' + params.page + '/' + params.pageSize + '?' + pm + '&pRF_REQ_TYPE_ID=' + '&pUSER_ID=' + cur_user_id).then(function (res) {
                            e.success(res);
                        });
                    }
                },
