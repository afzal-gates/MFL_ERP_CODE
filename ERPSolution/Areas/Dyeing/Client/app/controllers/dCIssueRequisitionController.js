...
(function () {
    'use strict';
    angular.module('multitex.dyeing').controller('DCIssueRequisitionController', ['$q', 'config', 'DyeingDataService', '$stateParams', '$state', '$scope', 'commonDataService', 'formData', 'cur_user_id', 'Dialog', '$filter', DCIssueRequisitionController]);
    function DCIssueRequisitionController($q, config, DyeingDataService, $stateParams, $state, $scope, commonDataService, formData, cur_user_id, Dialog, $filter) {
        ...
        vm.form = formData.DYE_STR_REQ_H_ID ? formData : { HR_DEPARTMENT_ID: '48', STR_REQ_BY: cur_user_id, STR_REQ_DT: vm.today, RF_ACTN_VIEW: 0 };
        ...
        vm.showSplash = true;
        function activate() {
            var promise = [getUserData(), GetItemCategoryList(), ItemList(), getCurrencyList(), getReceipeTemplateList(),
                ReceiveItemList(), GetReqSourceList(), GetReqTypeList(), GetCompanyList(), GetStatusList(), GetMOUList(),
                GetSupplierList(), GetSourceTypeList(), GetMachineList(), GetReqStoreList()];
                ...
                transport: {
                    read: function (e) {
                        DyeingDataService.getDataByFullUrl('/api/dye/LabdipRecipe/GetLDDosingTemplete').then(function (res) {
                            var list = _.filter(res, function (x) { return x.LK_DC_TMPLT_TYPE_ID != 536 && x.LK_DC_TMPLT_TYPE_ID != 499 })
                            e.success(list);
                        });
                    }
                },
        ...
        vm.checkStock = function () {
            if (parseFloat(preItem.RQD_QTY) > (parseFloat(preItem.CENTRAL_STK) + parseFloat(preItem.SUB_STK))) {
        ...
        function ReceiveItemList() {
            ...
            if ($stateParams.pDYE_STR_REQ_H_ID > 0) {
                ...
                DyeingDataService.getDataByFullUrl('/api/dye/LabdipRecipe/GetLDDosingTempleteDtl?pDYE_DOSE_TMPLT_H_ID=' + vm.form.DYE_DOSE_TMPLT_H_ID).then(function (output) {
                    ...
                    //DyeingDataService.getDataByFullUrl('/api/Dye/DCIssueRequisition/GetDCOtherTemplate?pLK_DC_CONS_TYPE_ID=500').then(function (output) {
        ...
        //            transport: {
        //                read: function (e) {
        //                    DyeingDataService.getDataByFullUrl('/api/common/GetStoreInfo?pINV_ITEM_CAT_LST=3,4').then(function (res) {
