...
(function () {
    'use strict';
    angular.module('multitex.dyeing').controller('ScProgramChallanListController', ['$q', 'config', 'DyeingDataService', '$stateParams', '$state', '$rootScope', '$scope', 'Hub', 'cur_user_id', '$modal', ScProgramChallanListController]);
    function ScProgramChallanListController($q, config, DyeingDataService, $stateParams, $state, $rootScope, $scope, Hub, cur_user_id, $modal) {
            ...
            return vm.gridOptionsDS = new kendo.data.DataSource({
                ...
                transport: {
                    read: function (e) {
                        ...
                        DyeingDataService.getDataByFullUrl('/api/dye/DfScProgram/ScChallanGetAll/' + params.page + '/' + params.pageSize + '?' + pm + '&pUSER_ID=' + cur_user_id).then(function (res) {
                            e.success(res);
                        });
                    }
                },
        ...
        vm.gridOptions = {
            ...
            columns: [
                ...
                { field: "GATE_PASS_NO", title: "Gate Pass No", type: "string", width: "10%" },
                {
                    title: "Action",
                    template: function () {
                        return '</a><a ui-sref="ScPreTreatmentChallan({pDF_SC_PRG_CHL_ISS_H_ID:dataItem.DF_SC_PRG_CHL_ISS_H_ID})" ng-if="(dataItem.ACTN_ROLE_FLAG==' + "'RI'" + ' || dataItem.ACTN_ROLE_FLAG==' + "'RA'" + ') && dataItem.PERMISSION==1" class="btn btn-xs green"><i class="fa fa-edit"> Edit</i></a> ' +
