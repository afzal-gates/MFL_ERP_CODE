(function () {
    'use strict';
    angular.module('multitex.dyeing').controller('DCIssuedLoanAdjustController', ['$q', 'config', 'DyeingDataService', '$stateParams', '$state', '$scope', 'commonDataService', 'formData', 'cur_user_id', '$modal', 'Dialog', DCIssuedLoanAdjustController]);
    function DCIssuedLoanAdjustController($q, config, DyeingDataService, $stateParams, $state, $scope, commonDataService, formData, cur_user_id, $modal, Dialog) {
        ...
        //vm.form = { HR_COMPANY_ID: 1, SCM_STORE_ID: '6', ITEM_RCV_BY: cur_user_id, RET_CHALAN_DT: vm.today, RF_ACTN_VIEW: 0, RF_REQ_TYPE_ID: 11 };
        vm.form = formData.DYE_DC_LRT_H_ID ? formData : { HR_COMPANY_ID: 1, SCM_STORE_ID: '6', ITEM_RCV_BY: cur_user_id, RET_CHALAN_DT: vm.today, RF_ACTN_VIEW: 0, RF_REQ_TYPE_ID: 11 };
        ...
        vm.checkStock = function (item) {
            var qty = parseFloat(item.QTY_PER_PACK) * parseFloat(item.PACK_RET_QTY);
            //if (parseFloat(item.LN_RTN_BAL_QTY) < qty) {
            //    item.PACK_RET_QTY = '';
            //    item.RET_QTY = '';
        ...
        vm.checkStockRevers = function (item) {
            var qty = parseFloat(item.RET_QTY) / parseFloat(item.QTY_PER_PACK);
            //if (parseFloat(item.LN_RTN_BAL_QTY) < qty) {
            //    item.PACK_RET_QTY = '';
            //    item.RET_QTY = '';
            //    config.appToastMsg("MULTI-005 Requisition quantity must less than stock quantity.");
        ...
        vm.getItemBrandName = function (item) {
            ...
            if (item.ADJ_BY_ITEM_ID > 0) {
                ...
                DyeingDataService.getDataByFullUrl('/api/dye/DyeChemicalStoreTransfer/GetStockInfo/' + item.ADJ_BY_ITEM_ID + '/' + vm.form.SCM_STORE_ID + '/' + vm.form.SCM_STORE_ID).then(function (res) {
        ...
        function ReceiveItemList() {
            vm.ReceiveItemList = new kendo.data.DataSource({
                transport: {
                    read: function (e) {
                        DyeingDataService.getDataByFullUrl('/api/Dye/DCReceiveableLoan/GetDCReceiveableLoanInfoDtlByID?pDYE_DC_LRT_H_ID=' + ($stateParams.pDYE_DC_LRT_H_ID || 0)).then(function (res) {
                            e.success(res);
                        });
                    }
                }
        ...
        vm.getSupplierLoanDtl = function (e) {
            if (fnValidateSub3() == true) {
                var obj = e.sender.dataItem(e.item);
                DyeingDataService.getDataByFullUrl('/api/dye/DCLoanReturn/GetSupplierLoanDtlByID/' + (obj.SCM_SUPPLIER_ID || 0) + '/' + vm.form.SCM_STORE_ID).then(function (res) {
                    var List = _.filter(res, function (x) { return x.LN_RTN_BAL_QTY > 0 })
                    vm.ReceivedLoanItemList = List;
                });
            } else {
        ...
        vm.addToGrid = function (item) {
            ...
            if (fnValidateSub() == true) {
                ...
                var count = _.filter(vm.ReceiveItemList.data(), function (o) {
                    return (o.DC_ITEM_ID === item.DC_ITEM_ID && o.QTY_PER_PACK === item.QTY_PER_PACK)
                }).length;

                if (count == 0) {

                    item.STOCK_QTY = item.STOCK_QTY / 1000;
        ...
        vm.gridOptionsItem = {
            ...
            columns: [
                ...
                { field: "DYE_DC_LRT_D_ID", hidden: true },
                ...
                { field: "LN_RTN_BAL_QTY", title: "Pending Loan Qty", type: "string", width: "10%" },
                { field: "QTY_PER_PACK", title: "Qty/Pack", editable:true, type: "string", width: "10%" },
                { field: "PACK_RET_QTY", title: "Received Pack", editable: true, type: "string", width: "10%" },
                { field: "RET_QTY", title: "Total Recv(Kg)", editable: true, type: "string", width: "10%" },
        ...
        vm.submitAll = function (dataOri) {
            ...
            if (fnValidate() == true) {
                ...
                data["XML_ADJ_D"] = DyeingDataService.xmlStringShort(vm.ReceiveItemList.data().map(function (o) {
                    return {
                        ...
                        DYE_DC_LRT_D_ID: o.DYE_DC_LRT_D_ID == null ? 0 : o.DYE_DC_LRT_D_ID,
                        ...
                        QTY_PER_PACK: o.QTY_PER_PACK == null ? 1 : o.QTY_PER_PACK,
                        PACK_RET_QTY: o.PACK_RET_QTY == null ? 1 : o.PACK_RET_QTY,
                        COST_PRICE: o.COST_PRICE == null ? 1 : o.COST_PRICE,
                        RET_QTY_K: o.RET_QTY_K == null ? 0 : o.RET_QTY_K,
                        RET_QTY_G: o.RET_QTY_G == null ? 0 : o.RET_QTY_G,
                        RET_QTY_M: o.RET_QTY_M == null ? 0 : o.RET_QTY_M,
                        RET_QTY: o.RET_QTY == null ? 0 : o.RET_QTY,
                        QTY_MOU_ID: o.QTY_MOU_ID == null ? 0 : o.QTY_MOU_ID
                    }
                }));

                var id = vm.form.DYE_DC_LRT_H_ID

                return DyeingDataService.saveDataByUrl(data, '/DCReceiveableLoan/Save').then(function (res) {
                    ...
                    if (res.success === false) {
                        ...
                        if (id > 0) {
                            ...
                            $state.go($state.current, { pDYE_DC_LRT_H_ID: res.data.OPDYE_DC_LRT_H_ID }, { reload: true });
                        }
                        else {
                            config.appToastMsg(res.data.PMSG);
                            if (parseInt(res.data.OPDYE_DC_LRT_H_ID) > 0) {
                                vm.form.DYE_STR_REQ_H_ID = res.data.OPDYE_DC_LRT_H_ID;
                                $state.go($state.current, { pDYE_DC_LRT_H_ID: res.data.OPDYE_DC_LRT_H_ID }, { reload: true });
                            }
                        }
                ...
                return DyeingDataService.saveDataByUrl(data, '/DCReceiveableLoan/Save').then(function (res) {
                    ...
                    if (res.success === false) {
                        ...
                        if (id > 0) {
                            ...
                            config.appToastMsg(res.data.PMSG);
                            if (parseInt(res.data.OPDYE_DC_LRT_H_ID) > 0) {
                                vm.form.DYE_DC_LRT_H_ID = res.data.OPDYE_DC_LRT_H_ID;
                                $state.go($state.current, { pDYE_DC_LRT_H_ID: res.data.OPDYE_DC_LRT_H_ID }, { reload: true });
                            }
                        }
