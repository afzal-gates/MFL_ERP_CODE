(function () {
    'use strict';
    angular.module('multitex.dyeing').controller('CompleteBatchReprocessController', ['$q', 'config', 'DyeingDataService', '$stateParams', '$state', '$scope', 'commonDataService', 'formData', 'cur_user_id', 'Dialog', CompleteBatchReprocessController]);
    function CompleteBatchReprocessController($q, config, DyeingDataService, $stateParams, $state, $scope, commonDataService, formData, cur_user_id, Dialog) {
        ...
        vm.form = formData.DYE_STR_REQ_H_ID ? formData : { REQ_STORE_ID: '4', ISS_STORE_ID: '6', STR_REQ_BY: cur_user_id, STR_REQ_DT: vm.today, RF_ACTN_VIEW: 0, DYE_RE_PROC_TYPE_ID: 1 };
        vm.formItem = { 'QTY_MOU_ID': '3', 'CENTRAL_STK': 0, 'SUB_STK': 0 };

        activate()
        vm.showSplash = true;
        function activate() {
            var promise = [GetBatchList(), getdosingTemplateList(), GetItemCategoryList(), getItemDataByCategory(), GetMachineList(), ReceiveItemList(), GetStatusList()];
        ...
        if ($stateParams.pADDITION_FLAG) {
            ...
            vm.form.REQ_TYPE_NAME = "Normal Dyeing";
            ...
            if (obj.DYE_BT_CARD_H_ID > 0) {
                if (fnValidateSub2() == true) {
                    ...
                    vm.dosingTemplateList = new kendo.data.DataSource({
                        transport: {
                            read: function (e) {
                                DyeingDataService.getDataByFullUrl('/api/dye/LabdipRecipe/GetLDDosingTempleteByColorGrpID?pMC_COLOR_GRP_ID=' + list[0].MC_COLOR_GRP_ID + '&LK_DYE_MTHD_ID=' + list[0].LK_DYE_MTHD_ID).then(function (res) {
                                    if (res.length == 1) {
        ...
        function getItemDataByCategory() {
            ...
            //    var list = res;
            //    DyeingDataService.getDataByFullUrl('/api/dye/LabdipRecipe/GetItemListByCatID?pINV_ITEM_CAT_ID=3').then(function (resD) {
            ...
            if (fnValidate() == true) {
                ...
                data["XML_REQ_D1"] = DyeingDataService.xmlStringShort(vm.BatchProgramList.data().map(function (o) {
                    return {
                        ...
                        DYE_MACHINE_ID: o.DYE_MACHINE_ID,
                ...
                data["XML_REQ_D2"] = DyeingDataService.xmlStringShort(vm.recipeItemList.map(function (o) {
                    return {
