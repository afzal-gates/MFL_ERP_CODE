(function () {
    'use strict';
    angular.module('multitex.dyeing').controller('DyeChemicalLoanReceiveListController', ['$q', 'config', 'DyeingDataService', '$stateParams', '$state', '$scope', '$filter', DyeChemicalLoanReceiveListController]);
    function DyeChemicalLoanReceiveListController($q, config, DyeingDataService, $stateParams, $state, $scope, $filter) {
            ...
            return vm.gridOptionsDS = new kendo.data.DataSource({
                ...
                transport: {
                    read: function (e) {
                        ...
                        if (pm.length > 0) {
                            DyeingDataService.getDataByFullUrl('/api/dye/DyeChemicalReceive/SelectAll/' + params.page + '/' + params.pageSize + '?pRF_REQ_TYPE_ID=2&' + pm).then(function (res) {
                                e.success(res);
                            });
                        }
                        else {
        ...
        vm.gridOptions = {
            ...
            columns: [
                ...
                { field: "REMARKS", title: "Remarks", type: "string", width: "10%" },
                {
                    title: "Action",
                    template: function () {
                        return '</a><a ui-sref="DyeChemicalLoanReceiveView({pDYE_DC_RCV_H_ID:dataItem.DYE_DC_RCV_H_ID})" class="btn btn-xs green"><i class="fa fa-eye"> View</i></a> <a ui-sref="DyeChemicalLoanReceive({pDYE_DC_RCV_H_ID:dataItem.DYE_DC_RCV_H_ID})"  ng-if="dataItem.IS_FINALIZED==' + "'N'" + '" class="btn btn-xs blue"><i class="fa fa-edit"> Edit</i></a></a>';
                    },
                    width: "10%"
                }
            ]
