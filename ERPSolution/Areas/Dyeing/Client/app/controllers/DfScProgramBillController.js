...
(function () {
    'use strict';
    angular.module('multitex.dyeing').controller('DfScProgramBillController', ['$q', 'config', 'DyeingDataService', '$stateParams', '$state', '$rootScope', '$scope', 'Hub', 'scoBillHdrData', 'cur_user_id', '$filter', 'Dialog', DfScProgramBillController]);
    function DfScProgramBillController($q, config, DyeingDataService, $stateParams, $state, $rootScope, $scope, Hub, scoBillHdrData, cur_user_id, $filter, Dialog) {
        ...
        vm.IS_PROG_PARAM = 'Y';
        vm.IS_NEXT = 'Y';
        var key = 'DF_SCO_FP_BILL_H_ID';
        vm.today = new Date();

        vm.form = scoBillHdrData[key] ? scoBillHdrData : {
            BILL_DT: vm.today, DF_SCO_FP_BILL_H_ID: 0, HR_COMPANY_ID: null, SCM_SUPPLIER_ID: null, BILL_NO: '', REMARKS: '', SCO_BILL_D_XML: '', REPORT_CODE: 'RPT-3563'
        };
        ...
        vm.delvChlnDataSource = new kendo.data.DataSource({
            ...
            transport: {
                read: function (e) {
                    ...
                    var url = '/api/Dye/DfScProgramBill/GetChallanList4Bill';
            ...
            angular.forEach(data, function (val, key) {
                ...
                if (val['IS_SELECT'] == 'Y') {
                    var chlnData = _.filter(billData, function (ob) {
                        return ob.DF_SCO_CHL_RCV_H_ID == val['DF_SCO_CHL_RCV_H_ID'];
                    });
                    ...
                    if (chlnData.length < 1) {
                        ...
                        angular.forEach(val['fabList'], function (val1, key1) {
                            ...
                            val1['CHLN_QTY'] = val1['RCV_FIN_FAB_QTY'];
                            val1['DELV_QTY'] = val1['RCV_GREY_FAB_QTY'];
                            val1['DEDUCT_QTY'] = 0; //val1['DELV_GFAB_WT'];
                            val1['NET_QTY'] = val1['RCV_FIN_FAB_QTY'] - val1['DEDUCT_QTY'];
                            val1['PROD_RATE'] = 0;
        ...
        vm.billDtlDataSource = new kendo.data.DataSource({
            transport: {
                read: function (e) {
                    ...
                    var url = '/api/dye/DfScProgramBill/GetBillDtl?pDF_SCO_FP_BILL_H_ID=' + ($stateParams.pDF_SCO_FP_BILL_H_ID || $scope.$parent.form.DF_SCO_FP_BILL_H_ID || 0);
            ...
            Dialog.confirm(vMsg, 'Are you sure?', ['Yes', 'No']).then(function () {
                ...
                submitData.XML_BILL_D = DyeingDataService.xmlStringShort(billDtlData.map(function (ob) {
                    return {
                        ...
                        DF_SCO_FP_BILL_H_ID: ob.DF_SCO_FP_BILL_H_ID,
                        DF_SCO_CHL_RCV_H_ID: ob.DF_SCO_CHL_RCV_H_ID,
                ...
                var id = vm.form.DF_SCO_FP_BILL_H_ID;

                return DyeingDataService.saveDataByUrl(submitData, '/DfScProgramBill/BatchSave').then(function (res) {
                    ...
                    if (res.success === false) {
                        ...
                        if (id > 0) {
                            ...
                            $state.go($state.current, { pDF_SCO_FP_BILL_H_ID: res.data.OPDF_SCO_FP_BILL_H_ID }, { inherit: false, reload: true });
                        }
                        else {
                            config.appToastMsg(res.data.PMSG);
                            if (parseInt(res.data.OPDF_SCO_FP_BILL_H_ID) > 0) {
                                vm.form.DF_SCO_FP_BILL_H_ID = res.data.OPDF_SCO_FP_BILL_H_ID;
                                $state.go($state.current, { pDF_SCO_FP_BILL_H_ID: res.data.OPDF_SCO_FP_BILL_H_ID }, { inherit: false, reload: true });
                            }
                        }
        ...
        vm.backToList = function () {
            return $state.go('DfScProgramBillList');
        };
        ...
        vm.reviseBillModal = function (scoBillHdrID) {
            ...
            modalInstance.result.then(function (data) {
                ...
                $state.go('ScoBillProcH.Dtl', { pDF_SCO_FP_BILL_H_ID: vRevisionData.DF_SCO_FP_BILL_H_ID });
