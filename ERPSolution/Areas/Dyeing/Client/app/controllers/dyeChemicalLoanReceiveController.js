(function () {
    'use strict';
    angular.module('multitex.dyeing').controller('DyeChemicalLoanReceiveController', ['$q', 'config', 'DyeingDataService', '$stateParams', '$state', '$scope', 'commonDataService', 'formData', 'cur_user_id', '$timeout', '$filter', DyeChemicalLoanReceiveController]);
    function DyeChemicalLoanReceiveController($q, config, DyeingDataService, $stateParams, $state, $scope, commonDataService, formData, cur_user_id, $timeout, $filter) {
        ...
        vm.showSplash = true;
        function activate() {
            var promise = [getUserData(), GetItemCategoryList(), ItemList(), getCurrencyList(), ReceiveItemList(), GetReqSourceList(), GetReqTypeList(), GetCompanyList(), GetPaymentMethodList(), GetMOUList(), GetPackMOUList(), GetSupplierList(), GetSourceTypeList()];
        ...
        vm.form = formData.DYE_DC_RCV_H_ID ? formData : { HR_COMPANY_ID: '1', SCM_STORE_ID: 6, RF_REQ_TYPE_ID: 2, DC_MRR_DT: vm.today, IS_LC_PNDNG: 'N', ITEM_RCV_BY: cur_user_id };
        ...
        vm.reset = function () {
            ...
            $state.go('DyeChemicalLoanReceive', { pDYE_DC_RCV_H_ID: 0 });
                    ...
                    transport: {
                        read: function (e) {
                            DyeingDataService.getDataByFullUrl('/api/common/GetReqType').then(function (res) {
                                var list = _.filter(res, function (o) { return o.IS_R_I == "R" && o.RF_REQ_TYPE_ID == 2 });
                                e.success(list);
                            });
                        }
                    }
                ...
                return DyeingDataService.saveDataByUrl(data, '/DyeChemicalReceive/Save').then(function (res) {
                    ...
                    if (res.success === false) {
                        ...
                        if (id > 0) {
                            ...
                            $state.go($state.current, { pDYE_DC_RCV_H_ID: id }, { reload: true });
                        }
                        else {
                            config.appToastMsg(res.data.PMSG);
