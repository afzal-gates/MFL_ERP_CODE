(function () {
    'use strict';
    angular.module('multitex.ie').controller('GmtWashReqListController', ['$q', 'config', 'IeDataService', '$stateParams', '$state', '$scope', 'cur_user_id', GmtWashReqListController]);
    function GmtWashReqListController($q, config, IeDataService, $stateParams, $state, $scope, cur_user_id) {
            ...
            return vm.gridOptionsDS = new kendo.data.DataSource({
                ...
                transport: {
                    read: function (e) {
                        var webapi = new kendo.data.transports.webapi({});
                        var params = webapi.parameterMap(e.data);
                        IeDataService.getDataByFullUrl('/api/ie/GmtWashReq/SelectAll/' + params.page + '/' + params.pageSize + '?' + pm + '&pUSER_ID=' + cur_user_id).then(function (res) {
                            e.success(res);
                        });
                    }
                },
            ...
            return vm.gridOptionsDS = new kendo.data.DataSource({
                ...
                transport: {
                    read: function (e) {
                        var webapi = new kendo.data.transports.webapi({});
                        var params = webapi.parameterMap(e.data);
                        var pm = IeDataService.kFilterStr2QueryParam(params.filter);
                        IeDataService.getDataByFullUrl('/api/ie/GmtWashReq/SelectAll/' + params.page + '/' + params.pageSize + '?' + pm + '&pUSER_ID=' + cur_user_id).then(function (res) {
                            e.success(res);
                        });
                    }
        ...
        vm.gridOptions = {
            ...
            columns: [
                ...
                },
                {
                    title: "Action",
                    template: function () {
                        return '</a><a ui-sref="GmtWashReq({pGMT_DF_WASH_REQ_ID:dataItem.GMT_DF_WASH_REQ_ID})" ng-if="(dataItem.ACTN_ROLE_FLAG==' + "'RI'" + ') && dataItem.PERMISSION==1" class="btn btn-xs blue"><i class="fa fa-edit"> {{dataItem.NXT_ACTION_NAME}}</i></a>' +
                            ' <a ng-click="vm.printRDLC(dataItem);" class="btn btn-xs yellow-gold"><i class="fa fa-print"> Print</i></a></a>';
                    },
                    width: "15%"
                }
            ]
        ...
        vm.deleteReq = function (dataItem) {
            ...
            Dialog.confirm('Are you want to delete all issue of "' + data.WASH_REQ_NO + '.', 'Attention', ['Yes', 'No'])
                 .then(function () {
                     vm.showSplash = true;

                     return IeDataService.saveDataByUrl(data, '/GmtWashReq/DeleteReq').then(function (res) {
