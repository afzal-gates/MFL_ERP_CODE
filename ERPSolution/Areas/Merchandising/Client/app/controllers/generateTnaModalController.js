(function () {
    ...
    angular.module('multitex.mrc').controller('GenerateTnaModalController', ['$modalInstance', '$scope', 'StyleData', 'Order', 'TnATaskList', '$q', 'config', '$filter', 'MrcDataService', 'Dialog', '$modal', 'cur_date_server', 'DeselectTnaTaskList', GenerateTnaModalController]);

    function GenerateTnaModalController($modalInstance, $scope, StyleData, Order, TnATaskList, $q, config, $filter, MrcDataService, Dialog, $modal, cur_date_server, DeselectTnaTaskList) {
        ...
        var leadTimeAct = moment(Order.SHIP_DT).diff(moment(Order.ORD_CONF_DT), 'days');
        var DayMulTi = 1;
        ...
        $scope.openTnaRevisionModal = function (data) {
            console.log(data);
            var modalInstance = $modal.open({
                animation: true,
                templateUrl: 'TNARevisionModal.html',
                controller: 'TnaRevisionModalController',
                size: 'small',
            ...
            modalInstance.result.then(function (data) {
                $scope.OrderCopy['IS_TNA_FINALIZED'] = data.IS_TNA_FINALIZED;
                $scope.Order['IS_TNA_FINALIZED'] = data.IS_TNA_FINALIZED;
            }, function () {
        ...
        //Same TnA for Related Style Extention
        $scope.sameTnaSave = function (token) {

            var submitData = $scope.Order;
           
            $scope.showSplash = true;
            return MrcDataService.saveDataByUrl(submitData, '/StyleH/SameOrderTnASave', token).then(function (res) {
                res['data'] = angular.fromJson(res.jsonStr);
                if (res.data.PMSG.substr(0, 9) == 'MULTI-001') {
                    $scope.showSplash = false;
                    //reload();
        ...
        //    if (ix != 0) {
        //        var tmp = $scope.TnATaskList[ix - 1];
        //        $scope.TnATaskList[ix - 1] = $scope.TnATaskList[ix];
        //        $scope.TnATaskList[ix] = tmp;

        //        angular.forEach($scope.TnATaskList, function (val, key) {
        //            val.TASK_ORDER = key + 1;
        //        });
        //    }
        ...
        //    if (ix > -1 && ix < $scope.TnATaskList.length - 1) {
        //        var tmp = $scope.TnATaskList[ix + 1];
        //        $scope.TnATaskList[ix + 1] = $scope.TnATaskList[ix];
        //        $scope.TnATaskList[ix] = tmp;
