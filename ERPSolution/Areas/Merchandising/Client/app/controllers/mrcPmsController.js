(function () {
    'use strict';
    angular.module('multitex.mrc').controller('MrcPmsController', ['$q', 'config', 'MrcDataService', '$stateParams', '$state', '$scope', 'BuyerAccData', MrcPmsController]);
    function MrcPmsController($q, config, MrcDataService, $stateParams, $state, $scope, BuyerAccData) {
        ...
        vm.onSelect = function (tab) {
            ...
            $state.go('pms.grid', { pMC_BYR_ACC_ID: tab.MC_BYR_ACC_ID, pFIRSTDATE: vm.form.FIRSTDATE, pLASTDATE: vm.form.LASTDATE, pMONTHOF: vm.form.MONTHOF }, { reload: 'pms.grid' });
        }
...
(function () {
    'use strict';
    angular.module('multitex.mrc').controller('MrcPmsDController', ['$q', 'config', 'MrcDataService', '$stateParams', '$state', '$scope', '$modal', '$window', MrcPmsDController]);
    function MrcPmsDController($q, config, MrcDataService, $stateParams, $state, $scope, $modal, $window) {
        ...
        vm.gridOptions = {
            ...
            columns: [
                ...
                { field: "FIB_COMP_CODE", title: "Fabrication", width: "180px", filterable: false, template: '# if (STYLE_ORD_SL == 1 ) { # <small title=\"#= FIB_COMP_CODE_FULL#\"><b>  #= FIB_COMP_CODE# </small></b> <span ng-click="vm.moreFabricListModal(dataItem.MC_FAB_PROD_ORD_H_ID)" style=\"display:block;cursor:pointer;\"><i class=\"fa fa-search-plus\">more</i></span>    # } #' },
