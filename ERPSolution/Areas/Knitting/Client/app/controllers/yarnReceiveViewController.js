(function () {
    'use strict';
    angular.module('multitex.knitting').controller('YarnReceiveViewController', ['$q', 'config', 'KnittingDataService', '$stateParams', '$state', '$scope', 'commonDataService', 'formData', 'cur_user_id', YarnReceiveViewController]);
    function YarnReceiveViewController($q, config, KnittingDataService, $stateParams, $state, $scope, commonDataService, formData, cur_user_id) {
        ...
        vm.showSplash = true;
        function activate() {
            var promise = [getUserData(), GetItemCategoryList(), ItemList(), getCurrencyList(), ReceiveItemList(), GetReqSourceList(), GetReqTypeList(), GetPriorityList(), getTnaStatusList(), GetCompanyList(), GetPaymentMethodList(), getFiscalYearData(), GetYarnColorGroupList(), GetMOUList(), GetSupplierList(), GetSourceTypeList()];
        ...
        vm.form = formData.KNT_YRN_RCV_H_ID ? formData : { HR_COMPANY_ID: 1, SCM_STORE_ID: 1, IS_LC_PNDNG: 'N', INV_ITEM_CAT_ID: '2', ITEM_RCV_BY: cur_user_id, HR_EMPLOYEE_ID: cur_user_id, YRN_MRR_DT: vm.today };
        ...
        $scope.YRN_MRR_DT_LNopen = function ($event) {
            $event.preventDefault();
            $event.stopPropagation();
            $scope.YRN_MRR_DT_LNopened = true;
        }
        ...
        function ReceiveItemList() {
            vm.ReceiveItemList = new kendo.data.DataSource({
                transport: {
                    read: function (e) {
                        KnittingDataService.getDataByFullUrl('/api/knit/YarnReceive/GetYarnReceiveInfoDtlByID?pKNT_YRN_RCV_H_ID=' + ($stateParams.pKNT_YRN_RCV_H_ID || 0)).then(function (res) {
                            e.success(res);
                        });
                    }
                }
        ...
        vm.addToGrid = function (isValid) {
            ...
            if (fnValidateSub() == true) {
                ...
                var ItemID = vm.formItem.YARN_ITEM_ID;
                var brandID = vm.formItem.RF_BRAND_ID;
                var ColorGrpID = vm.formItem.LK_YRN_COLR_GRP_ID;
                ...
                if (vm.formItem.uid) {
                    ...
                    if (count <= 1) {
                        ...
                        row["KNT_YRN_RCV_H_ID"] = vm.formItem.KNT_YRN_RCV_H_ID;
                        ...
                        vm.formItem = { 'YARN_ITEM_ID': ItemID, 'QTY_MOU_ID': '3', 'LK_YRN_COLR_GRP_ID': ColorGrpID, 'RF_BRAND_ID': brandID, 'PACK_MOU_ID': packID };
                    ...
                    var count = _.filter(vm.ReceiveItemList.data(), function (o) {
                        return (o.YARN_ITEM_ID === vm.formItem.YARN_ITEM_ID && o.RF_BRAND_ID === vm.formItem.RF_BRAND_ID && o.LK_YRN_COLR_GRP_ID === vm.formItem.LK_YRN_COLR_GRP_ID && o.YRN_LOT_NO === vm.formItem.YRN_LOT_NO)
                    }).length;
        ...
        vm.reset = function () {
            ...
            $state.go('YarnReceive', { pKNT_YRN_RCV_H_ID: 0 });
        ...
        vm.submitAll = function (dataOri) {
            ...
            if (fnValidate() == true) {
                ...
                var id = vm.form.KNT_YRN_RCV_H_ID

                return KnittingDataService.saveDataByUrl(data, '/YarnReceive/UpdateLC').then(function (res) {
                    ...
                    if (res.success === false) {
                        ...
                        if (parseInt(id) > 0) {
                            ...
                            $state.go('YarnReceiveView', { pKNT_YRN_RCV_H_ID: id }, { reload: true });
                        }
                        else {
                            config.appToastMsg(res.data.PMSG);
                            if (parseInt(res.data.OPKNT_YRN_RCV_H_ID) > 0) {
                                vm.form.KNT_YRN_RCV_H_ID = res.data.OPKNT_YRN_RCV_H_ID;
                                $state.go($state.current, { pKNT_YRN_RCV_H_ID: res.data.OPKNT_YRN_RCV_H_ID }, { reload: true });
                            }
                        }
