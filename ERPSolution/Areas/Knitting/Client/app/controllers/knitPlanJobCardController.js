(function () {
    'use strict';
    angular.module('multitex.knitting').controller('KnitPlanJobCardController', ['$q', 'config', 'KnittingDataService', '$stateParams', '$state', '$scope', 'JobCardHeader', 'Dialog', 'YarnLotList', '$modal', '$window', KnitPlanJobCardController]);
    function KnitPlanJobCardController($q, config, KnittingDataService, $stateParams, $state, $scope, JobCardHeader, Dialog, YarnLotList, $modal, $window) {
        ...
        function getJobCardList() {
            if ($stateParams.pKNT_SC_PRG_ISS_ID) {
                return KnittingDataService.getDataByUrl('/KnitPlan/getJobCardList?pKNT_SC_PRG_ISS_ID=' + $stateParams.pKNT_SC_PRG_ISS_ID).then(function (res) {
                    vm.jobCardList = res;
                    vm.ENABLED_REQS = _.every(_.filter(res, function (o) {
            ...
            if (!valid1 || !valid2 || validYarnLot || details.length == 0) {
                ...
                return KnittingDataService.saveDataByUrl(data, '/KnitPlan/SaveJobCardData').then(function (res) {
                    if (res.success === false) {
                        ...
                        if (res.data.PMSG.substr(0, 9) == 'MULTI-001') {
                            $state.go('KnitPlanJobCard', { pKNT_JOB_CRD_H_ID: parseInt(res.data.OP_KNT_JOB_CRD_H_ID) }, { reload: true });
        ...
        function openUnAssignQtyModal(itm) {
            ...
            modalInstance.result.then(function (data) {
                if (data.success) {
                    $state.go('KnitPlanJobCard', {}, { inherit: true, reload:true });
                }
        ...
        vm.unAssignQty = function (itm) {
            ...
            //if (itm.REQ_YRN_QTY_DONE > 0) {
            //    Dialog.confirm("You can't cancel the knit card because yarn requisition already done againest it.", "Warning", ['Ok']);
            //}
            //else {
                Dialog.confirm('Do you really want to cancel or un-assign qty from job card?<br> Click <b>Yes</b> to proceed', 'Attention', ['Yes', 'No']).then(function () {
        ...
        vm.openExColourListModal = function (data) {
                ...
                resolve: {
                    Params: function () {
                        return {
                            ...
                            pMC_FAB_PROD_ORD_H_ID: data.MC_FAB_PROD_ORD_H_ID,
                            pRF_FAB_PROD_CAT_ID: data.RF_FAB_PROD_CAT_ID,
                            LK_COL_TYPE_ID_LST:'358,361,407,432', //NaviCfg[$stateParams.state]['LK_COL_TYPE_ID_LST'],
                            DEFAULT_CAT_ID: '2',//NaviCfg[$stateParams.state]['DEFAULT_CAT_ID'],
                            RF_FAB_PROD_CAT_ID_LST: '2,4,5,6,8,9'//NaviCfg[$stateParams.state]['RF_FAB_PROD_CAT_ID_LST']
                        }
                    }
                }
            });
            ...
            modalInstance.result.then(function (data) {
                if (data.KNT_PLAN_H_ID) {
                    if ($stateParams.pKNT_SC_PRG_ISS_ID) {
                        $state.go('KnitPlanJobCard', { pKNT_PLAN_H_ID: data.KNT_PLAN_H_ID, pKNT_JOB_CRD_H_ID: 0 }, {inherit:true});
                    } else {
                        $state.go('KnitPlanJobCard', { pKNT_PLAN_H_ID: data.KNT_PLAN_H_ID }, { inherit: false });
                    }
        ...
        vm.onEditClick = function (item) {

            $state.go('KnitPlanJobCard', {
                pKNT_PLAN_H_ID: $stateParams.pKNT_PLAN_H_ID,
                state: $stateParams.state,
                pKNT_JOB_CRD_H_ID : item.KNT_JOB_CRD_H_ID,
                pMC_FAB_PROD_ORD_H_ID: $stateParams.pMC_FAB_PROD_ORD_H_ID,
                pRF_FAB_PROD_CAT_ID: $stateParams.RF_FAB_PROD_CAT_ID
            });
        };
