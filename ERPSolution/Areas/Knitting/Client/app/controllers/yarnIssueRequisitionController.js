(function () {
    'use strict';
    angular.module('multitex.knitting').controller('YarnIssueRequisitionController', ['$q', 'config', 'KnittingDataService', '$stateParams', '$state', '$scope', 'commonDataService', 'formData', 'cur_user_id', '$filter', YarnIssueRequisitionController]);
    function YarnIssueRequisitionController($q, config, KnittingDataService, $stateParams, $state, $scope, commonDataService, formData, cur_user_id, $filter) {
        ...
        vm.form = formData.KNT_YRN_STR_REQ_H_ID ? formData : { INV_ITEM_CAT_ID: '2', HR_COMPANY_ID: 1, SCM_STORE_ID: 1, STR_REQ_BY: cur_user_id, STR_REQ_DT: vm.today, RF_ACTN_VIEW: 0 };
        vm.formItem = {};
        ...
        function activate() {
            var promise = [
                getBuyerAcGrpList(), getStyleListData(), RequisitionList(), getUserData(), GetItemCategoryList(),
                ItemList(), YarnLotList(), GetReqTypeList(), GetReqStoreList(), GetCompanyList(),
                GetStatusList(), GetYarnColorGroupList(), GetMOUList(), GetSupplierList()
            ];
                ...
                transport: {
                    read: function (e) {
                        ...
                        return KnittingDataService.getDataByFullUrl(url).then(function (res) {
                            //var list = _.filter(res, function (o) { return o.MC_BYR_ACC_GRP_ID === 0 })
        ...
        vm.TotalRequiredQty = function () {
            if (vm.formItem.LOOSE_QTY < vm.formItem.QTY_PER_PACK)
                ...
                vm.formItem.RQD_YRN_QTY = parseFloat((vm.formItem.PACK_RQD_QTY || 0)) * parseFloat(vm.formItem.QTY_PER_PACK);
            }
        };

        vm.checkStockQty = function () {
            if (parseInt(vm.formItem.STOCK_QTY) < (vm.formItem.RQD_YRN_QTY)) {
                vm.formItem.RQD_YRN_QTY = '';
            }
        }
        ...
        function ItemList() {
            return vm.DyItemList = new kendo.data.DataSource({
                transport: {
                    read: function (e) {
                        KnittingDataService.getDataByFullUrl('/api/knit/YarnReceive/GetKnitSTKItemListByCatID?pINV_ITEM_CAT_ID=2').then(function (res) {
                            e.success(res);
                        });
                    }
                }
        ...
        vm.ItemListByType = function () {
            return vm.DyItemList = new kendo.data.DataSource({
                transport: {
                    read: function (e) {
                        KnittingDataService.getDataByFullUrl('/api/knit/YarnReceive/GetKnitSTKItemListByCatID?pINV_ITEM_CAT_ID=2&pRF_REQ_TYPE_ID=' + vm.form.RF_REQ_TYPE_ID).then(function (res) {
                            e.success(res);
                        });
                    }
                }
        ...
        vm.getItemLotStock = function (e) {
            ...
            vm.formItem.MOU_CODE = data[0].MOU_CODE;
            vm.formItem.PACK_MOU_ID = data[0].PACK_MOU_ID;
        ...
        vm.getItemStock = function (e) {
            ...
            if (fnValidateSub2() == true) {
                var obj = e.sender.dataItem(e.item);
                KnittingDataService.getDataByFullUrl('/api/knit/YarnIssueReq/ItemLotStockByID/' + (obj.INV_ITEM_ID || 0) + '/' + (vm.form.SCM_STORE_ID || 0) + '/' + (vm.form.RF_REQ_TYPE_ID || 0)).then(function (res) {

                    vm.yarnLotList = new kendo.data.DataSource({
                        data: res.data
                    });

                    //var bList = _.filter(res, { 'BRAND_NAME_EN': 'T' });

                    //for (var j = 0; j < res.length; j++) {

                    //    var d = 0;
                    //    for (var x = 0; x < bList.length; x++) {
                    //        if (bList[x].BRAND_NAME_EN == res[j].BRAND_NAME_EN)
                    //            d = 1;
                    //        break;
                    //    }
                    ...
                    //var cList = _.filter(res, { 'BRAND_NAME_EN': 'T' });
                    //for (var i = 0; i < res.length; i++) {

                    //    var c = 0;
                    //    for (var y = 0; y < cList.length; y++) {
                    //        if (cList[y].BRAND_NAME_EN == res[i].BRAND_NAME_EN)
                    //            c = 1;
                    //        break;
                    //    }
        ...
        vm.getItemDataByCategory = function (e) {
            ...
            KnittingDataService.getDataByFullUrl('/api/knit/YarnReceive/GetKnitSTKItemListByCatID?pINV_ITEM_CAT_ID=' + (itemss.INV_ITEM_CAT_ID || 2)).then(function (res) {
        ...
        vm.addToGrid = function (isValid) {
            ...
            if (fnValidateSub() == true) {
                var item = _.filter(vm.DyItemList.data(), function (o) {
                    return o.INV_ITEM_ID === parseInt(vm.formItem.YARN_ITEM_ID)
                })[0];
                ...
                vm.formItem.MC_FAB_PROD_ORD_H_ID = vm.form.MC_FAB_PROD_ORD_H_ID;
                ...
                if (vm.formItem.uid) {
                    ...
                    if (count <= 1) {
                        ...
                        row["MC_FAB_PROD_ORD_H_ID"] = vm.formItem.MC_FAB_PROD_ORD_H_ID;
                        ...
                        row["RQD_CONE_QTY"] = vm.formItem.RQD_CONE_QTY;
        ...
        vm.brandWiseLotNo = function () {
            KnittingDataService.getDataByFullUrl('/api/knit/YarnIssueReq/ItemLotStockByID/' + (vm.formItem.YARN_ITEM_ID || 0) + '/' + (vm.form.SCM_STORE_ID || 0) + '/' + (vm.form.RF_REQ_TYPE_ID || 0)).then(function (res) {

                var lst = _.filter(res.data, function (x) { return x.RF_BRAND_ID == vm.formItem.RF_BRAND_ID })
                vm.yarnLotList = new kendo.data.DataSource({
                    data: lst
                });

                var clst = _.filter(res.data, function (x) { return x.RF_BRAND_ID == vm.formItem.RF_BRAND_ID })
                vm.yarnColorGroupList = new kendo.data.DataSource({
                    data: clst
                });
        ...
        vm.editItemData = function (data) {
            ...
            vm.form.MC_FAB_PROD_ORD_H_ID = edata.MC_FAB_PROD_ORD_H_ID;
        ...
        function GetReqTypeList() {
            ...
            return vm.reqTypeList = {
                ...
                dataSource: {
                    transport: {
                        read: function (e) {
                            KnittingDataService.getDataByFullUrl('/api/common/GetReqTypeByUser').then(function (res) {
        ...
        vm.gridOptionsItem = {
            ...
            columns: [
                ...
                { field: "PACK_RQD_QTY", hidden: true },
                { field: "LOOSE_QTY", hidden: true },
                ...
                { field: "RQD_CONE_QTY", title: "Reqd Cone", type: "string", width: "7%" },
                { field: "RQD_YRN_QTY", title: "Reqd Knit (Kg)", type: "string", width: "7%" },
                ...
                return KnittingDataService.saveDataByUrl(data, '/YarnIssueReq/Save').then(function (res) {
                    ...
                    if (res.success === false) {
                        ...
                        if (id > 0) {
                            ...
                            $state.go('YarnIssueReq', { pKNT_YRN_STR_REQ_H_ID: id }, { reload: true });
                        }
                        else {
