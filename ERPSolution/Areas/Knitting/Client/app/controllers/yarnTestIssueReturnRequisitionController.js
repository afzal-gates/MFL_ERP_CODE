(function () {
    'use strict';
    angular.module('multitex.knitting').controller('YarnTestIssueReturnRequisitionController', ['$q', 'config', 'KnittingDataService', '$stateParams', '$state', '$scope', 'commonDataService', 'formData', 'cur_user_id', 'Dialog', YarnTestIssueReturnRequisitionController]);
    function YarnTestIssueReturnRequisitionController($q, config, KnittingDataService, $stateParams, $state, $scope, commonDataService, formData, cur_user_id, Dialog) {
        ...
        vm.form = formData.KNT_YRN_ISS_RET_H_ID ? formData : { INV_ITEM_CAT_ID: '2', ITEM_RET_BY: cur_user_id, RET_CHALAN_DT: vm.today };
        vm.formItem = {};
        ...
        function RequisitionList() {
            KnittingDataService.getDataByFullUrl('/api/knit/YarnIssueReturn/GetYarnStoreReq?pRF_REQ_TYPE_ID=4&pUSER_ID=' + cur_user_id).then(function (res) {
            ...
            KnittingDataService.getDataByFullUrl('/api/knit/YarnIssueReturn/GetYarnRtnDtlByID?pKNT_YRN_ISS_RET_H_ID=' + ($stateParams.pKNT_YRN_ISS_RET_H_ID || 0)).then(function (res) {
                vm.ReturnItemList = new kendo.data.DataSource({
                    data: res
                });
            });
        };
        ...
        vm.getRequisitionByType = function (e) {
            ...
            return vm.requisitionList = new kendo.data.DataSource({
                transport: {
                    read: function (e) {
                        KnittingDataService.getDataByFullUrl('/api/knit/YarnIssueReturn/GetYarnStoreReq?pRF_REQ_TYPE_ID=' + (obj.RF_REQ_TYPE_ID || null)).then(function (res) {
                            e.success(res);
                        });
                    }
                }
        ...
        vm.reset = function () {
            ...
            $state.go('YarnTestIssueRtnReq', { pKNT_YRN_ISS_RET_H_ID: 0 });
                ...
                return KnittingDataService.saveDataByUrl(data, '/YarnIssueReturn/Save').then(function (res) {
                    ...
                    if (res.success === false) {
                        ...
                        if (id > 0) {
                            ...
                            $state.go($state.current, { pKNT_YRN_ISS_RET_H_ID: id }, { reload: true });
                            //$state.go('YarnTestIssueRtnReq', { pKNT_YRN_ISS_RET_H_ID: id });
                        }
                        else {
                            ...
                            if (parseInt(res.data.OPKNT_YRN_ISS_RET_H_ID) > 0) {
                                ...
                                $state.go($state.current, { pKNT_YRN_ISS_RET_H_ID: res.data.OPKNT_YRN_ISS_RET_H_ID }, { reload: true });
                            }
                        }
