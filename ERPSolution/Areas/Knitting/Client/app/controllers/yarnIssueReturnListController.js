(function () {
    'use strict';
    angular.module('multitex.knitting').controller('YarnIssueReturnListController', ['$q', 'config', 'KnittingDataService', '$stateParams', '$state', '$scope', 'cur_user_id', YarnIssueReturnListController]);
    function YarnIssueReturnListController($q, config, KnittingDataService, $stateParams, $state, $scope, cur_user_id) {
            ...
            dataSource: new kendo.data.DataSource({
                ...
                transport: {
                    read: function (e) {
                        ...
                        KnittingDataService.getDataByFullUrl('/api/knit/YarnIssueReturn/SelectAll/' + params.page + '/' + params.pageSize + '?' + pm + '&pUSER_ID=' + cur_user_id + '&pOption=3003').then(function (res) {
                            e.success(res);
                        });
                    }
                },
                ...
                },
                {
                    ...
                    template: function () {
                        return '</a><a ui-sref="YarnIssueRtnReq({pKNT_YRN_ISS_RET_H_ID:dataItem.KNT_YRN_ISS_RET_H_ID})" ng-if="(dataItem.ACTN_ROLE_FLAG==' + "'RI'" + ' || dataItem.ACTN_ROLE_FLAG==' + "'RA'" + ') && dataItem.PERMISSION==1" class="btn btn-xs blue"><i class="fa fa-edit"> {{dataItem.NXT_ACTION_NAME}}</i></a> ' +
                            '<a ui-sref="YarnIssueRtn({pKNT_YRN_ISS_RET_H_ID:dataItem.KNT_YRN_ISS_RET_H_ID})" ng-if="dataItem.ACTN_ROLE_FLAG==' + "'SR'" + ' && dataItem.PERMISSION==1" class="btn btn-xs blue"><i class="fa fa-edit"> {{dataItem.NXT_ACTION_NAME}}</i></a> </a>';
