/// <reference path="knitPlanYdController.js" />
(function () {
    'use strict';
    angular.module('multitex.knitting').controller('KnitPlanYdController', ['$q', 'config', 'KnittingDataService', '$stateParams', '$state', '$scope', 'KnitPlanHeader', 'Dialog', '$modal', '$modalInstance', KnitPlanYdController]);
    function KnitPlanYdController($q, config, KnittingDataService, $stateParams, $state, $scope, KnitPlanHeader, Dialog, $modal, $modalInstance) {
        ...
        function getFabricDetails() {
            ...
            url += '&pHAS_YD=1&pLK_COL_TYPE_ID_LST=' + ($stateParams.pCT_ID_LST || '358,361,407,432') + '&pRF_FAB_TYPE_ID=' + (($stateParams.state === 'FabProdKnitOrderFBR') ? 5 : null);;
        ...
        var yd_colors;
        vm.onChangeIsSolidCol = function (item) {

            console.log(item);

            if (item.IS_SOLID_COL == 'Y') {                
                //yd_colors = angular.copy(KnitPlanHeader['yd_colors']);
                item.yd_colors = item.solid_colors || KnitPlanHeader['solid_colors'];
                console.log(item.yd_colors);
            } else {
                item.yd_colors = KnitPlanHeader['yd_colors'];
            }
        ...
        vm.addToListKpFeed = function (data, list) {
            var d2push = {
                ...
                yd_colors: angular.copy(data.yd_colors),
                solid_colors: angular.copy(data.solid_colors)
            };
            list.push(d2push);
        };
        ...
        vm.openProgramModal = function (yyyy) {
            ...
            modalInstance.result.then(function (dta) {
                ...
                if (dta.KNT_SC_PRG_ISS_ID > 0) {
                    $state.go('KnitPlanJobCardYd', { pKNT_PLAN_H_ID: yyyy.KNT_PLAN_H_ID, pKNT_SC_PRG_ISS_ID: dta.KNT_SC_PRG_ISS_ID, pIS_SUB_CONTR: 'Y' });
                }
        ...
        vm.submitData = function (dataOri, valid, IS_REVISED) {
            ...
            angular.forEach(dataOri.kp_feeders, function (val, key) {
                if ((parseInt(val.SOLID_COLOR_ID || 0) <= 0) || (parseInt(val.SOLID_COLOR_ID == 3) && val['REMARKS']=='')) {
        ...
        vm.createStoreRequisition = function (pKNT_PLAN_H_ID) {
            Dialog.confirm('Do you really want to submit Yarn Requisition?<br> Click <b>Yes</b> to proceed', 'Attention', ['Yes', 'No'])
                .then(function () {
                    return KnittingDataService.saveDataByUrl({}, '/KnitPlan/CreateStoreRequisitionYdKp?pKNT_PLAN_H_ID=' + pKNT_PLAN_H_ID).then(function (res) {
                        res['data'] = angular.fromJson(res.jsonStr);
                        if (res.data.PMSG.substr(0, 9) == 'MULTI-001') {
                            refreshData();
                            getFabricDetails();
