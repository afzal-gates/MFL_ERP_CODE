(function () {
    'use strict';
    angular.module('multitex.knitting').controller('KnitMcNeedleRcvListController', ['$q', 'config', 'KnittingDataService', '$stateParams', '$state', '$scope', KnitMcNeedleRcvListController]);
    function KnitMcNeedleRcvListController($q, config, KnittingDataService, $stateParams, $state, $scope) {
        ...
        vm.gridOptions = {
            dataSource: new kendo.data.DataSource({
                ...
                transport: {
                    read: function (e) {
                        ...
                        KnittingDataService.getDataByFullUrl('/api/knit/KnitMcNeedleRcv/SelectAll/' + params.page + '/' + params.pageSize + '?' + pm).then(function (res) {
                            e.success(res);
                        });
                    }
                },
            ...
            columns: [
                ...
                { field: "REMARKS", title: "Remarks", type: "string", width: "15%" },
                {
                    title: "Action",
                    template: function () {
                        return '</a><a ui-sref="KnitMcNeedleRcvView({pKNT_NDL_STR_RCV_H_ID:dataItem.KNT_NDL_STR_RCV_H_ID})" class="btn btn-xs green"><i class="fa fa-eye"> View</i></a> <a ui-sref="KnitMcNeedleRcv({pKNT_NDL_STR_RCV_H_ID:dataItem.KNT_NDL_STR_RCV_H_ID})" ng-if="dataItem.IS_FINALIZED==' + "'N'" + '" class="btn btn-xs blue"><i class="fa fa-edit"> Edit</i></a></a>';
                    },
                    width: "10%"
                }
