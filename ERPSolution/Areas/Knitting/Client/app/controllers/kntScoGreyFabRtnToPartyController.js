...
(function () {
    'use strict';
    angular.module('multitex.knitting').controller('KntScoGreyFabRtnToPartyHController', ['logger', 'config', '$q', '$scope', '$http', 'exception', '$filter', '$state', '$stateParams', 'formData', 'formScoRejFabList', 'KnittingDataService', 'Dialog', KntScoGreyFabRtnToPartyHController]);
    function KntScoGreyFabRtnToPartyHController(logger, config, $q, $scope, $http, exception, $filter, $state, $stateParams, formData, formScoRejFabList, KnittingDataService, Dialog) {
        ...
        formData['rejFabDtl'] = formScoRejFabList;
        var key = 'KNT_SCO_GFAB_RET_H_ID';
        
        console.log(formData);
        console.log($stateParams);
        console.log(formData['KNT_SCO_GFAB_RET_H_ID']);
        //var rcvDate = moment($stateParams.pRCV_DT).format("DD-MMM-YYYY");

        vm.today = new Date();
        vm.form = formData[key] ? formData : { KNT_SCO_GFAB_RET_H_ID: 0, RET_CHALAN_DT: moment(vm.today).format("DD-MMM-YYYY"), rejFabDtl: [], fabRtnDtl: [], yrnRcvDtl: [] }; //formData[key] ? formData : { KNT_SC_GFAB_DLV_H_ID: 0, SCM_SUPPLIER_ID: 0, CHALAN_NO: null, CHALAN_DT: vm.today, GT_PASS_NO: '', GFAB_DLV_DTL_XML: '', IS_FINALIZE: 'N' };
        $scope.form = formData[key] ? formData : { KNT_SCO_GFAB_RET_H_ID: 0, RET_CHALAN_DT: moment(vm.today).format("DD-MMM-YYYY"), rejFabDtl: [], fabRtnDtl: [], yrnRcvDtl: [] }; //formData[key] ? formData : { KNT_SC_GFAB_DLV_H_ID: 0, SCM_SUPPLIER_ID: 0, CHALAN_NO: null, CHALAN_DT: vm.today, GT_PASS_NO: '', GFAB_DLV_DTL_XML: '', IS_FINALIZE: 'N' };

        //vm.IS_NEXT = 'N';
        $scope.isFabAddToGrid = 'Y';
        $scope.selectedFabRow = {};
           
        vm.fabRcvState = ($state.current.name === 'ScoGreyFabRtnToPartyH.FabricReturn') ? true : false;
        ...
        vm.gotoNext = function () {
            ...
            vm.fabRcvState = true;
            vm.IS_NEXT = 'Y';
            $state.go('ScoGreyFabRtnToPartyH.FabricReturn', { pKNT_SCO_GFAB_RET_H_ID: 0, pSCM_SUPPLIER_ID: vm.form.SCM_SUPPLIER_ID }, { notify: false });
            

            return KnittingDataService.getDataByFullUrl('/api/Knit/KntScChlnRtn/GetScoQcRejFabList4Rtn?&pSCM_SUPPLIER_ID=' + (vm.form.SCM_SUPPLIER_ID || 0)).then(function (res) {
                 vm.form.rejFabDtl = res;
                 //$scope.form.rejFabDtl = res;
        ...
        vm.batchSave = function (pIS_FINALIZED) {
            ...
            Dialog.confirm(vMsg, 'Confirmation', ['Yes', 'No']).then(function () {
                ...
                return KnittingDataService.saveDataByFullUrl(submitRcvData, '/api/knit/KntScChlnRtn/BatchSave4ScoFabRtn').then(function (res) {
                    console.log(res);
                    ...
                    if (res.success === false) {
                        ...
                        if (res.data.PMSG.substr(0, 9) == 'MULTI-001') {
                            ...
                            //vm.getChallanList();
                            if ((res.data.PKNT_SCO_GFAB_RET_H_ID_RTN * 1) > 0) {

                                //alert('x');
                                $stateParams.pKNT_SCO_GFAB_RET_H_ID = res.data.PKNT_SCO_GFAB_RET_H_ID_RTN;
                                $scope.form.KNT_SCO_GFAB_RET_H_ID = res.data.PKNT_SCO_GFAB_RET_H_ID_RTN;
                                ...
                                $state.go('ScoGreyFabRtnToPartyH.FabricReturn', { pKNT_SCO_GFAB_RET_H_ID: res.data.PKNT_SCO_GFAB_RET_H_ID_RTN, pSCM_SUPPLIER_ID: $stateParams.pSCM_SUPPLIER_ID }, { reload: true });

                                //if (vm.yrnRcvState == true) {
                                //    $state.go('ScoGreyFabRtnToPartyH.YrnRcv', { pPKNT_SCO_GFAB_RET_H_ID_RTN: res.data.PKNT_SCO_GFAB_RET_H_ID_RTN, pSCM_SUPPLIER_ID: $stateParams.pSCM_SUPPLIER_ID }, { reload: 'ScGreyFabRcvFormPartyH.YrnRcv' });
                                //}
                                //else {
                                //    $state.go('ScoGreyFabRtnToPartyH.FabricReturn', { pPKNT_SCO_GFAB_RET_H_ID_RTN: res.data.PKNT_SCO_GFAB_RET_H_ID_RTN, pSCM_SUPPLIER_ID: $stateParams.pSCM_SUPPLIER_ID }, { reload: 'ScoGreyFabRtnToPartyH.FabricReturn' });
                                //}
        ...
        vm.cancel = function () {
            ...
            $state.go('ScoGreyFabRtnToPartyH.FabricReturn', { pKNT_SCO_GFAB_RET_H_ID: 0, pSCM_SUPPLIER_ID: $scope.form.SCM_SUPPLIER_ID });

            $scope.form = { KNT_SCO_CHL_RCV_H_ID: 0, RCV_DT: moment(vm.today).format("DD-MMM-YYYY"), rejFabDtl: [], fabRtnDtl: [], yrnRcvDtl: [] };
        }

        vm.backToList = function () {
            return $state.go('ScoGreyFabRtnToPartyList', { pSCM_SUPPLIER_ID: $scope.form.SCM_SUPPLIER_ID });
        };
...
(function () {
    'use strict';
    angular.module('multitex.knitting').controller('KntScoGreyFabRtnToPartyFabRtnController', ['logger', 'config', '$q', '$scope', '$http', 'exception', '$filter', '$state', '$stateParams', 'KnittingDataService', 'Dialog', KntScoGreyFabRtnToPartyFabRtnController]);
    function KntScoGreyFabRtnToPartyFabRtnController(logger, config, $q, $scope, $http, exception, $filter, $state, $stateParams, KnittingDataService, Dialog) {
        ...
        vm.addFabRcv = function (fabItem) {
            $scope.$parent.form.fabRtnDtl.push({
                KNT_SCO_GFAB_RET_D_ID: 0, KNT_SCO_GFAB_RET_H_ID: $stateParams.pKNT_SCO_GFAB_RET_H_ID, KNT_STYL_FAB_ITEM_ID: $scope.$parent.selectedFabRow.KNT_STYL_FAB_ITEM_ID,
                ...
                RET_GFAB_QTY: vm.fabItem.RET_GFAB_QTY, QTY_MOU_ID: 3, PROD_RATE: vm.fabItem.PROD_RATE, NOTE: vm.fabItem.NOTE
            });
...
(function () {
    'use strict';
    angular.module('multitex.knitting').controller('KntScoGreyFabRtnToPartyListController', ['logger', 'config', '$q', '$scope', '$http', 'exception', '$filter', '$state', '$stateParams', 'KnittingDataService', 'Dialog', KntScoGreyFabRtnToPartyListController]);
    function KntScoGreyFabRtnToPartyListController(logger, config, $q, $scope, $http, exception, $filter, $state, $stateParams, KnittingDataService, Dialog) {
        ...
        vm.today = new Date();
        vm.form = { KNT_SCO_GFAB_RET_H_ID: 0, RET_CHALAN_DT: moment(vm.today).format("DD-MMM-YYYY"), rejFabDtl: [], yrnRcvDtl: [] }; //formData[key] ? formData : { KNT_SC_GFAB_DLV_H_ID: 0, SCM_SUPPLIER_ID: 0, CHALAN_NO: null, CHALAN_DT: vm.today, GT_PASS_NO: '', GFAB_DLV_DTL_XML: '', IS_FINALIZE: 'N' };
            ...
            vm.partyChallanDataSource = new kendo.data.DataSource({
                ...
                transport: {
                    read: function (e) {
                        var webapi = new kendo.data.transports.webapi({});
                        var params = webapi.parameterMap(e.data);
                        var url = '/api/knit/KntScChlnRtn/GetScoRtnChallanList?pSCM_SUPPLIER_ID=' + vm.form.SCM_SUPPLIER_ID;
        ...
        vm.partyChallanOptions = {
            ...
            columns: [
                ...
                { field: "GATE_PASS_NO", title: "Gate Pass#", type: "string", hidden: true },
                ...
                { field: "REMARKS", title: "Remarks", type: "string", hidden: true },
                {
                    title: "Action",
                    template: function () {                        
                        return "<a class='btn btn-xs blue' ui-sref='ScoGreyFabRtnToPartyH.FabricReturn({pKNT_SCO_GFAB_RET_H_ID:dataItem.KNT_SCO_GFAB_RET_H_ID, pSCM_SUPPLIER_ID:dataItem.SCM_SUPPLIER_ID })' ><i class='fa fa-edit'> Edit</i></a>&nbsp;<a ng-click='vm.printChallan(dataItem)' class='btn btn-xs blue'><i class='fa fa-print'> Print</i></a>";
                    },
                    width: "250px"
                }          
            ]
        ...
        vm.newChallan = function () {            
            $state.go('ScoGreyFabRtnToPartyH.FabricReturn', { pKNT_SCO_GFAB_RET_H_ID: 0, pSCM_SUPPLIER_ID: vm.form.SCM_SUPPLIER_ID });
        }
