(function () {
    'use strict';
    angular.module('multitex.knitting').controller('YarnReceiveController', ['$q', 'config', 'KnittingDataService', '$stateParams', '$state', '$scope', 'commonDataService', 'formData', 'cur_user_id', YarnReceiveController]);
    function YarnReceiveController($q, config, KnittingDataService, $stateParams, $state, $scope, commonDataService, formData, cur_user_id) {
        ...
        function activate() {
            var promise = [getUserData(), GetItemCategoryList(), ItemList(), getCurrencyList(), ReceiveItemList(), GetReqSourceList(), GetReqTypeList(),
                GetPriorityList(), getTnaStatusList(), GetCompanyList(), GetPaymentMethodList(), getFiscalYearData(), GetYarnColorGroupList(), GetMOUList(),
                GetSupplierList(), GetSourceTypeList()];
        ...
        vm.confirmAll = function (dataOri) {
            ...
            if (fnValidate() == true) {
                ...
                data["XML_RCV_D"] = KnittingDataService.xmlStringShort(vm.ReceiveItemList.data().map(function (o) {
                    return {
                        KNT_YRN_RCV_D_ID: o.KNT_YRN_RCV_D_ID == null ? 0 : o.KNT_YRN_RCV_D_ID,
                        KNT_YRN_RCV_H_ID: o.KNT_YRN_RCV_H_ID == null ? 0 : o.KNT_YRN_RCV_H_ID,
                ...
                return KnittingDataService.saveDataByUrl(data, '/YarnReceive/Save').then(function (res) {
                    ...
                    if (res.success === false) {
                        ...
                        if (parseInt(id) > 0) {
                            ...
                            $state.go('YarnReceive', { pKNT_YRN_RCV_H_ID: id }, { reload: true });
                        }
                        else {
                            config.appToastMsg(res.data.PMSG);
