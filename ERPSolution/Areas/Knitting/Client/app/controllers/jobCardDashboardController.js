(function () {
    'use strict';
    angular.module('multitex.knitting').controller('JobCardDashboardController', ['$q', 'config', 'KnittingDataService', '$stateParams', '$state', '$scope', JobCardDashboardController]);
    function JobCardDashboardController($q, config, KnittingDataService, $stateParams, $state, $scope) {
        ...
        function activate() {
            var promise = [getSelectMonth(0, 0), getStyleExtList(null, $state.current.data.DEFAULT_CAT_ID, null, null), getFabOederByOh(null, $state.current.data.DEFAULT_CAT_ID, null, null, ''), showGrid(0, ($stateParams.pMC_FAB_PROD_ORD_H_ID || 0), $state.current.data.DEFAULT_CAT_ID, null)];
        ...
        $scope.search = {
            MC_BYR_ACC_ID: 0,
            RF_FAB_PROD_CAT_ID: $state.current.data.DEFAULT_CAT_ID,
        ...
        vm.showFabProdOrderList = function () {
            ...
            $stateParams.pMC_FAB_PROD_ORD_H_ID = -1;
            showGrid(($scope.search.MC_BYR_ACC_ID || 0), ($scope.search.MC_FAB_PROD_ORD_H_ID || 0), ($scope.search.RF_FAB_PROD_CAT_ID || 0), ($scope.search.FIRSTDATE||null))
        }
        ...
        function showGrid(MC_BYR_ACC_ID, MC_FAB_PROD_ORD_H_ID, RF_FAB_PROD_CAT_ID, FIRSTDATE) {
            return vm.gridOptionsDs = new kendo.data.DataSource({
                ...
                transport: {
                    read: function (e) {
                        var webapi = new kendo.data.transports.webapi({});
                        var params = webapi.parameterMap(e.data);
                        KnittingDataService.getDataByFullUrl('/api/knit/FabProdKnitOrder/SelectAll/' + params.page + '/' + params.pageSize + '/' + MC_BYR_ACC_ID + '/' + MC_FAB_PROD_ORD_H_ID + '/' + RF_FAB_PROD_CAT_ID + '/null/' + FIRSTDATE+'?pHAS_KP=Y&pLK_COL_TYPE_ID_LST=' + $state.current.data.LK_COL_TYPE_ID_LST).then(function (res) {
                            e.success(res);
                        });
                    }
                },
        ...
        vm.detailGridOptionsForSupp = function (dataItem) {
            return {
                dataSource: {
                    transport: {
                        read: function (e) {
                            KnittingDataService.getDataByFullUrl('/api/knit/FabProdKnitOrder/SelectByID?pMC_FAB_PROD_ORD_H_ID=' + dataItem.MC_FAB_PROD_ORD_H_ID + '&pLK_COL_TYPE_ID_LST=' + dataItem.LK_COL_TYPE_ID_LST).then(function (res) {
                                e.success(_.filter(res, function (o) {
                                    return o.KNT_PLAN_H_ID != -1;
                                }))
                            });
                        }
                    },
                ...
                { field: "FIB_COMP_NAME", title: "Fiber Comp", width: "15%" },
...

                {
                    title: "Action",
                    template: function () {
                        return "</a><a href='/Knitting/Knit/KnitPlan/#/JobCard?pKNT_PLAN_H_ID={{dataItem.KNT_PLAN_H_ID}}' class='btn btn-xs blue' ng-disabled='IS_ACTIVE==\"N\"'><i class='fa fa-plus'>Knit Card</i></a></a>";
                    },
                    width: "7%"
                }
