(function () {
    'use strict';
    angular.module('multitex.knitting').controller('YarnTestIssueController', ['$q', 'config', 'KnittingDataService', '$stateParams', '$state', '$scope', 'commonDataService', 'formData', 'cur_user_id', YarnTestIssueController]);
    function YarnTestIssueController($q, config, KnittingDataService, $stateParams, $state, $scope, commonDataService, formData, cur_user_id) {
        ...
        vm.form = formData.KNT_YRN_ISS_H_ID ? formData : { KNT_YRN_STR_REQ_H_ID: '0', INV_ITEM_CAT_ID: '2', ITEM_ISS_BY: cur_user_id, IS_FINALIZED: 'N', ISS_CHALAN_DT: vm.today };

        vm.form.RF_REQ_TYPE_ID = 4;
        vm.form.KNT_YRN_STR_REQ_H_ID = ($stateParams.pKNT_YRN_STR_REQ_H_ID || 0) == 0 ? vm.form.KNT_YRN_STR_REQ_H_ID : ($stateParams.pKNT_YRN_STR_REQ_H_ID || 0);

        activate()
        vm.showSplash = true;
        function activate() {
            var promise = [RequisitionList(), getUserData(), GetItemCategoryList(), ItemList(), YarnLotList(), IssueItemList(), GetReqSourceList(), GetReqTypeList(), GetStatusList(), GetCompanyList(), GetYarnColorGroupList(), GetMOUList()];
            return $q.all(promise).then(function () {
                vm.showSplash = false;
            });
        }
        //vm.form.KNT_YRN_STR_REQ_H_ID = ($stateParams.pKNT_YRN_STR_REQ_H_ID||0);
        ...
        vm.TotalIssueQty = function (index) {
            if (parseFloat(vm.IssueItemList[index].QTY_PER_PACK) < parseFloat(vm.IssueItemList[index].LOOSE_QTY)) {
                vm.IssueItemList[index].LOOSE_QTY = "";
                vm.IssueItemList[index].ISSUE_QTY = parseFloat(vm.IssueItemList[index].PACK_ISS_QTY * vm.IssueItemList[index].QTY_PER_PACK) + parseFloat(vm.IssueItemList[index].LOOSE_QTY || 0);

                return;
            }
            if (vm.IssueItemList[index].PACK_ISS_QTY >= 0 && vm.IssueItemList[index].QTY_PER_PACK > 0)
                vm.IssueItemList[index].ISSUE_QTY = parseFloat(vm.IssueItemList[index].PACK_ISS_QTY * vm.IssueItemList[index].QTY_PER_PACK) + parseFloat(vm.IssueItemList[index].LOOSE_QTY || 0);
            else
        ...
        function IssueItemList() {
            if (vm.form.KNT_YRN_ISS_H_ID > 0) {
                ...
                KnittingDataService.getDataByFullUrl('/api/knit/YarnIssueReq/GetYarnIssueReqInfo?pKNT_YRN_STR_REQ_H_ID=' + (vm.form.KNT_YRN_STR_REQ_H_ID || 0)).then(function (res) {
                    vm.form.STR_REQ_DT = res.STR_REQ_DT;
                    //vm.form.RF_REQ_TYPE_ID = obj.RF_REQ_TYPE_ID;
                    vm.form.USER_NAME_EN = res.USER_NAME_EN;
                    vm.form.HR_COMPANY_ID = res.HR_COMPANY_ID;
                    vm.form.SCM_STORE_ID = res.SCM_STORE_ID;
                });
        ...
        vm.reset = function () {
            ...
            $state.go('YarnTestIssue', { pKNT_YRN_ISS_H_ID: 0 });
        ...
        vm.submitAll = function (dataOri) {
            ...
            if (fnValidate() == true) {
                ...
                data["XML_ISS_D"] = KnittingDataService.xmlStringShort(vm.IssueItemList.map(function (o) {
                    return {
                        ...
                        ADV_ISSUE_QTY: o.ISSUE_QTY - o.REQ_QTY,
                ...
                return KnittingDataService.saveDataByUrl(data, '/YarnIssue/Save').then(function (res) {
                    ...
                    if (res.success === false) {
                        ...
                        if (id > 0) {
                            ...
                            $state.go($state.current, { pKNT_YRN_ISS_H_ID: res.data.OPKNT_YRN_ISS_H_ID }, { reload: true });
                        }
                        else {
                            config.appToastMsg(res.data.PMSG);
