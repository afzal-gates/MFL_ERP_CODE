...
(function () {
    'use strict';
    angular.module('multitex.cmr').controller('ExpLcPIController', ['$q', 'config', 'CmrDataService', 'commonDataService', '$stateParams', '$state', '$rootScope', '$scope', 'cur_user_id', '$filter', 'Dialog', '$modal', 'formData', ExpLcPIController]);
    function ExpLcPIController($q, config, CmrDataService, commonDataService, $stateParams, $state, $rootScope, $scope, cur_user_id, $filter, Dialog, $modal, formData) {
        ...
        vm.form = formData.CM_EXP_PI_H_ID ? formData : { DOC_TITLE: 'Export PI No', RF_CURRENCY_ID: 2, IS_ADV_LC: 'N', IS_LC_ADJ: 'N' };
        activate()
        vm.showSplash = true;
        function activate() {
        ...
        if (formData) {
            ...
            vm.exportOrderList = new kendo.data.DataSource({
                transport: {
                    read: function (e) {
                        CmrDataService.getDataByFullUrl('/api/cmr/ExportLCPI/GetPOListByID?pCM_EXP_PI_H_ID=' + (formData.CM_EXP_PI_H_ID || 1)).then(function (res) {
                            e.success(res);

                            var ttl_po = 0;
                            for (var i = 0; i < res.length; i++) {
                                var obj = res[i];
                                ttl_po = ttl_po + (obj.ORDER_QTY * obj.UNIT_PRICE).toFixed(2);
                            }
                            vm.form.PO_AMT = parseFloat(ttl_po);
                        });
                    }
                }
            });

            vm.form["ATTN_BUYER"] = formData.ATTN_BUYER ? formData.ATTN_BUYER.split(',') : [];
        ...
        vm.AddToGrid = function () {
            ...
            for (var i = 0; i < vm.exportOrderList.data().length; i++) {
                var obj = vm.orderDtlList.data()[i];
                ttl_po = ttl_po + LC_VAL;
            }
            vm.form.PO_AMT = ttl_po;
        ...
        vm.updateAll = function (dataOri) {
            ...
            if (fnValidate() == true) {
                ...
                var id = vm.form.CM_EXP_PI_H_ID;

                return CmrDataService.saveDataByUrl(data, '/ExportLCPI/Save').then(function (res) {
                    ...
                    if (res.success === false) {
                        ...
                        vm.form.CM_EXP_PI_H_ID = res.data.OPCM_EXP_PI_H_ID;
                        $state.go($state.current, { pCM_EXP_PI_H_ID: res.data.OPCM_EXP_PI_H_ID, pREVISE: 0 }, { inherit: false, reload: true })
                    }
                });
            }
        };
        ...
        vm.reviseAll = function (dataOri) {
            ...
            if (fnValidate() == true) {
                ...
                return CmrDataService.saveDataByUrl(data, '/ExportLCPI/Update').then(function (res) {
