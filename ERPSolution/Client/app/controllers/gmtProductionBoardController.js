(function () {
    'use strict';
    angular.module('multitex.displayboard').controller('GmtProductionBoardController', ['$q', '$stateParams', '$state', '$scope', '$modal', '$localStorage', 'Hub', '$rootScope', '$window', 'DisplayboardDataService', '$filter', GmtProductionBoardController]);
    function GmtProductionBoardController($q, $stateParams, $state, $scope, $modal, $localStorage, Hub, $rootScope, $window, DisplayboardDataService, $filter) {
        var vm = this;
            ...
            if (d.prods) {
                ...
                if (idx < 0) {
                    ...
                    if ((d.prods[idx].PROD_QTY / d.HRLY_TARGET) <= 0.85) {
                        return (d.prods[idx]['REASON_DESC_EN'] || 'NPT Not Entry');
                    } else {
                        return null;
                    }
                }
       ...
       function executedFromServer() {
        ...
        function getBoardData(pHR_PROD_FLR_LST, pPROD_DT) {
            ...
            if (!pPROD_DT) {
                $scope.dtSelected = new Date().toISOString();
                $('#start').text($filter('date')(new Date(),'dd-MMM-yyyy'));
            } else {
                $scope.dtSelected = pPROD_DT;
            }
           
            DisplayboardDataService.getDataByFullUrl('/api/ie/GmtIeTarget/GetDataForTargetSettingBoard?pRF_CALENDAR_DATE=' + $scope.dtSelected + '&pHR_PROD_FLR_LST=' + HR_PROD_FLR_LST).then(function (res) {
                vm.boardData = res;
            });
        }
