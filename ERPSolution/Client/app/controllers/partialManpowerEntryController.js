(function () {
    'use strict';
    angular.module('multitex').controller('PartialManpowerEntryController', ['$q', '$stateParams', '$state', '$scope', '$modal', '$localStorage', 'DashBoardService', 'FloorData', 'config', 'exception', 'cur_date_server', '$filter', PartialManpowerEntryController]);
    function PartialManpowerEntryController($q, $stateParams, $state, $scope, $modal, $localStorage, DashBoardService, FloorData, config, exception, cur_date_server, $filter) {
        ...
        vm.OnChangeFloor = function (itm) {
            ...
            $state.go('PartialManpowerEntry', {}, { notify: false, inherit: false });
        ...
        function getLineListData(pHR_PROD_FLR_LST,pHR_PROD_LINE_LST,DT) {
            return DashBoardService.getDataByUrl('/api/common/getDataOfMapowerAdjustment?pHR_PROD_FLR_LST=' + pHR_PROD_FLR_LST + '&pHR_PROD_LINE_LST=' + (pHR_PROD_LINE_LST || '') + '&pPROD_DT=' + DT).then(function (res) {
        ...
        $scope.$watch('vm.FloorLineData', function (newVal, oldVal) {
            ...
            vm.H15_P_WH_TTL = _.sumBy(newVal, function (o) { return (o.H15_P_WH || 0); });
        }, true);
        ...
        vm.save = function (dataOri,isValid) {
          ...
          var  data2bSave = dataOri.map(function (item) {
              return    {
                  GMT_LN_MP_ADJ_ID: item.GMT_LN_MP_ADJ_ID,
                  ...
                  H15_P_WH: item.H15_P_WH                  
              }
          });

           
          return DashBoardService.saveDataByUrl({ XML: config.xmlStringShort(data2bSave), PROD_DT: vm.selectedDt }, '/api/common/SaveGmtLineManpowerAdjustment').then(function (res) {
