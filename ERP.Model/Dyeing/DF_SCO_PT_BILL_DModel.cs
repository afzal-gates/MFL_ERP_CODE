...
namespace ERP.Model
{
    public class DF_SCO_PT_BILL_DModel
    {
        public Int64 DF_SCO_PT_BILL_D_ID { get; set; }
        ...
        public string Save()
        {
            const string sp = "SP_DF_SCO_PT_BILL_D";
            string jsonStr = "{";
            var ob = this;
            var i = 1;
            try
            {
                ...
                {
                     new CommandParameter() {ParameterName = "pDF_SCO_PT_BILL_D_ID", Value = ob.DF_SCO_PT_BILL_D_ID},
        ...
        public List<DF_SCO_PT_BILL_DModel> SelectAll()
        {
            string sp = "PKG_DF_PT.DF_SCO_PT_BILL_D_SELECT";
            try
            {
                var obList = new List<DF_SCO_PT_BILL_DModel>();
                OraDatabase db = new OraDatabase();
                var ds = db.ExecuteStoredProcedure(new List<CommandParameter>()
                {
                     new CommandParameter() {ParameterName = "pDF_SCO_PT_BILL_D_ID", Value =0},
                     new CommandParameter() {ParameterName = "pOption", Value =3000},
                     new CommandParameter() {ParameterName = "pMsg", Value =500, Direction = ParameterDirection.Output}
                 }, sp);
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    DF_SCO_PT_BILL_DModel ob = new DF_SCO_PT_BILL_DModel();

                    ob.DF_SCO_PT_BILL_D_ID = (dr["DF_SCO_PT_BILL_D_ID"] == DBNull.Value) ? 0 : Convert.ToInt64(dr["DF_SCO_PT_BILL_D_ID"]);
        ...
        public List<DF_SCO_PT_BILL_DModel> SelectByID(Int64? pDF_SCO_PT_BILL_H_ID = null)
        {
            string sp = "PKG_DF_PT.DF_SCO_PT_BILL_D_SELECT";
            try
            {
        ...
        public DF_SCO_PT_BILL_DModel Select(long ID)
        {
            string sp = "PKG_DF_PT.DF_SCO_PT_BILL_D_SELECT";
            try
            {
                var ob = new DF_SCO_PT_BILL_DModel();
                OraDatabase db = new OraDatabase();
                var ds = db.ExecuteStoredProcedure(new List<CommandParameter>()
                {
                     new CommandParameter() {ParameterName = "pDF_SCO_PT_BILL_D_ID", Value =0},
