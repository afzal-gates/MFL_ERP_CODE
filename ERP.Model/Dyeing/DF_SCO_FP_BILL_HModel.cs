...
namespace ERP.Model
{
    public class DF_SCO_FP_BILL_HModel
    {
        ...
        public string Save()
        {
            const string sp = "PKG_DF_SC_PROGRAM.DF_SCO_FP_BILL_H_INSERT";
            string jsonStr = "{";
            var ob = this;
            var i = 1;
            try
            {
                ...
                var ds = db.ExecuteStoredProcedure(new List<CommandParameter>()
                {
                     ...
                     new CommandParameter() {ParameterName = "opDF_SCO_FP_BILL_H_ID", Value =0, Direction = ParameterDirection.Output}
                 }, sp);
        ...
        {
            const string sp = "SP_DF_SCO_FP_BILL_H";
            string jsonStr = "{";
            var ob = this;
            var i = 1;
            try
        ...
        public List<DF_SCO_FP_BILL_HModel> SelectAll(int pageNo, int pageSize, Int64 pSCM_SUPPLIER_ID, string pBILL_NO = null, DateTime? pBILL_DT = null)
        {
            string sp = "PKG_DF_SC_PROGRAM.DF_SCO_FP_BILL_H_Select";
            try
            {
                var obList = new List<DF_SCO_FP_BILL_HModel>();
                OraDatabase db = new OraDatabase();
                var ds = db.ExecuteStoredProcedure(new List<CommandParameter>()
                {
                ...
                {
                    DF_SCO_FP_BILL_HModel ob = new DF_SCO_FP_BILL_HModel();
        ...
        public DF_SCO_FP_BILL_HModel Select(Int64? pDF_SCO_FP_BILL_H_ID = null)
        {
            string sp = "PKG_DF_SC_PROGRAM.DF_SCO_FP_BILL_H_Select";
            try
            {
                var ob = new DF_SCO_FP_BILL_HModel();
                OraDatabase db = new OraDatabase();
                var ds = db.ExecuteStoredProcedure(new List<CommandParameter>()
                {
