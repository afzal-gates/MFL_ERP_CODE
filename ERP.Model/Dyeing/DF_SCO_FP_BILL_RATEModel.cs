...
namespace ERP.Model
{
    public class DF_SCO_FP_BILL_RATEModel
    {
        public Int64 DF_SCO_FP_BILL_RATE_ID { get; set; }
        ...
        public string Save()
        {
            const string sp = "SP_DF_SCO_FP_BILL_RATE";
            string jsonStr="{";
            var ob = this;
            var i = 1;
            try
             {
                ...
                {
                     new CommandParameter() {ParameterName = "pDF_SCO_FP_BILL_RATE_ID", Value = ob.DF_SCO_FP_BILL_RATE_ID},
        ...
        public List<DF_SCO_FP_BILL_RATEModel> SelectAll()
        {
            string sp = "Select_DF_SCO_FP_BILL_RATE";
            try
            {
                var obList = new List<DF_SCO_FP_BILL_RATEModel>();
                OraDatabase db = new OraDatabase();
                var ds = db.ExecuteStoredProcedure(new List<CommandParameter>()
                {
                     new CommandParameter() {ParameterName = "pDF_SCO_FP_BILL_RATE_ID", Value =0},
                     new CommandParameter() {ParameterName = "pOption", Value =3000},
                     new CommandParameter() {ParameterName = "pMsg", Value =500, Direction = ParameterDirection.Output}
                 }, sp);
              foreach (DataRow dr in ds.Tables[0].Rows)
               {
                            DF_SCO_FP_BILL_RATEModel ob = new DF_SCO_FP_BILL_RATEModel();
                            ob.DF_SCO_FP_BILL_RATE_ID = (dr["DF_SCO_FP_BILL_RATE_ID"] == DBNull.Value) ? 0 : Convert.ToInt64(dr["DF_SCO_FP_BILL_RATE_ID"]);
        ...
        public DF_SCO_FP_BILL_RATEModel Select(long ID)
        {
            string sp = "Select_DF_SCO_FP_BILL_RATE";
            try
            {
                var ob = new DF_SCO_FP_BILL_RATEModel();
                OraDatabase db = new OraDatabase();
                var ds = db.ExecuteStoredProcedure(new List<CommandParameter>()
                {
                     new CommandParameter() {ParameterName = "pDF_SCO_FP_BILL_RATE_ID", Value =0},
