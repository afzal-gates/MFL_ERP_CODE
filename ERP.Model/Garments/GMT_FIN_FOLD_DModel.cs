...
namespace ERP.Model
{
    public class GMT_FIN_FOLD_DModel
    {
        public Int64 GMT_FIN_FOLD_D_ID { get; set; }
        public Int64 GMT_FIN_FOLD_H_ID { get; set; }
        public Int64 MC_ORDER_SIZE_ID { get; set; }
        public Int64 FOLD_QTY { get; set; }
        ...
        public List<GMT_FIN_FOLD_DModel> SelectAll()
        {
            string sp = "Select_GMT_FIN_FOLD_D";
            try
            {
                var obList = new List<GMT_FIN_FOLD_DModel>();
                OraDatabase db = new OraDatabase();
                var ds = db.ExecuteStoredProcedure(new List<CommandParameter>()
                {
                     new CommandParameter() {ParameterName = "pGMT_FIN_FOLD_D_ID", Value =0},
                     new CommandParameter() {ParameterName = "pOption", Value =3000},
                     new CommandParameter() {ParameterName = "pMsg", Value =500, Direction = ParameterDirection.Output}
                 }, sp);
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    GMT_FIN_FOLD_DModel ob = new GMT_FIN_FOLD_DModel();
                    ob.GMT_FIN_FOLD_D_ID = (dr["GMT_FIN_FOLD_D_ID"] == DBNull.Value) ? 0 : Convert.ToInt64(dr["GMT_FIN_FOLD_D_ID"]);
                    ...
                    ob.FOLD_QTY = (dr["FOLD_QTY"] == DBNull.Value) ? 0 : Convert.ToInt64(dr["FOLD_QTY"]);
        ...
        public GMT_FIN_FOLD_DModel Select(long ID)
        {
            string sp = "Select_GMT_FIN_FOLD_D";
            try
            {
                var ob = new GMT_FIN_FOLD_DModel();
                OraDatabase db = new OraDatabase();
                var ds = db.ExecuteStoredProcedure(new List<CommandParameter>()
                {
                     new CommandParameter() {ParameterName = "pGMT_FIN_FOLD_D_ID", Value =0},
