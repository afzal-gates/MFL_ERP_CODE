...
namespace ERP.Model
{
    public class GMT_FIN_IRON_DModel
    {
        public Int64 GMT_FIN_IRON_D_ID { get; set; }
        ...
        public List<GMT_FIN_IRON_DModel> SelectAll()
        {
            string sp = "Select_GMT_FIN_IRON_D";
            try
            {
                var obList = new List<GMT_FIN_IRON_DModel>();
                OraDatabase db = new OraDatabase();
                var ds = db.ExecuteStoredProcedure(new List<CommandParameter>()
                {
                     new CommandParameter() {ParameterName = "pGMT_FIN_IRON_D_ID", Value =0},
                     new CommandParameter() {ParameterName = "pOption", Value =3000},
                     new CommandParameter() {ParameterName = "pMsg", Value =500, Direction = ParameterDirection.Output}
                 }, sp);
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    GMT_FIN_IRON_DModel ob = new GMT_FIN_IRON_DModel();
                    ob.GMT_FIN_IRON_D_ID = (dr["GMT_FIN_IRON_D_ID"] == DBNull.Value) ? 0 : Convert.ToInt64(dr["GMT_FIN_IRON_D_ID"]);
        ...
        public GMT_FIN_IRON_DModel Select(long ID)
        {
            string sp = "Select_GMT_FIN_IRON_D";
            try
            {
                var ob = new GMT_FIN_IRON_DModel();
                OraDatabase db = new OraDatabase();
                var ds = db.ExecuteStoredProcedure(new List<CommandParameter>()
                {
                     new CommandParameter() {ParameterName = "pGMT_FIN_IRON_D_ID", Value =0},
