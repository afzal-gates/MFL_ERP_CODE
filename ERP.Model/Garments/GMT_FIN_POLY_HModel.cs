...
namespace ERP.Model
{
    public class GMT_FIN_POLY_HModel
    {
        ...
        public string GMT_FIN_POLY_H_XML { get; set; }
        public string GMT_FIN_POLY_D_XML { get; set; }

        public List<ExpandoObject> itemsOrder { get; set; }


        private List<GMT_FIN_POLY_DModel> _itemsGmtFinPolyDtl = null;
        public List<GMT_FIN_POLY_DModel> itemsGmtFinPolyDtl
        {
            get
            {
                if (_itemsGmtFinPolyDtl == null)
                {
                    _itemsGmtFinPolyDtl = new List<GMT_FIN_POLY_DModel>();
                }
                return _itemsGmtFinPolyDtl;
            }
            set
            {
                _itemsGmtFinPolyDtl = value;
            }
        }


        public string BatchSaveGmtFinPoly()
        {
            const string sp = "pkg_gmt_fin.gmt_fin_poly_save";
            string jsonStr = "{";
            var ob = this;
            var i = 1;
            try
            {
                ...
                {
                     new CommandParameter() {ParameterName = "pGMT_FIN_POLY_H_XML", Value = ob.GMT_FIN_POLY_H_XML},
                     new CommandParameter() {ParameterName = "pGMT_FIN_POLY_D_XML", Value = ob.GMT_FIN_POLY_D_XML},
        ...
        public List<ExpandoObject> GetGmtFinPolyOrderList(Int64? pGMT_FIN_POLY_H_ID, string pMC_ORDER_H_LST)
        {
            string sp = "pkg_gmt_fin.gmt_fin_poly_h_select";
            try
            {
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    foreach (DataRow dr1 in ds1.Tables[0].Rows)
                    {
                        ...
                        var ds2 = db.ExecuteStoredProcedure(new List<CommandParameter>()
                        {
                             ...
                             new CommandParameter() {ParameterName = "pGMT_FIN_POLY_H_ID", Value =pGMT_FIN_POLY_H_ID},
                        ...
                        foreach (DataRow dr2 in ds2.Tables[0].Rows)
                        {
                            ...
                            obOrdSize.GMT_FIN_POLY_D_ID = (dr2["GMT_FIN_POLY_D_ID"] == DBNull.Value) ? 0 : Convert.ToInt64(dr2["GMT_FIN_POLY_D_ID"]);
                            ...
                            obOrdSize.GMT_FIN_POLY_H_ID = (dr2["GMT_FIN_POLY_H_ID"] == DBNull.Value) ? 0 : Convert.ToInt64(dr2["GMT_FIN_POLY_H_ID"]);
                            ...
                            obOrdSize.POLY_QTY = (dr2["POLY_QTY"] == DBNull.Value) ? 0 : Convert.ToInt64(dr2["POLY_QTY"]);
        ...
        public List<GMT_FIN_POLY_HModel> GetGmtFinPolyData(DateTime pCALENDAR_DT, string pHR_PROD_FLR_LST)
        {
            string sp = "pkg_gmt_fin.gmt_fin_poly_h_select";
            try
            {
                var obList = new List<GMT_FIN_POLY_HModel>();
                OraDatabase db = new OraDatabase();
                var ds = db.ExecuteStoredProcedure(new List<CommandParameter>()
                {
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    GMT_FIN_POLY_HModel ob = new GMT_FIN_POLY_HModel();
                    ...
                    ob.itemsOrder = ob.GetGmtFinPolyOrderList(ob.GMT_FIN_POLY_H_ID, ob.MC_ORDER_H_LST);
