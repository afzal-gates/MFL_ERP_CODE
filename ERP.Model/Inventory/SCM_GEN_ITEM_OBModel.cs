...
namespace ERP.Model
{
    public class SCM_GEN_ITEM_OBModel
    {
        public Int64 SCM_GEN_ITEM_OB_ID { get; set; }
        ...
        public object XML_OB { get; set; }


        public string Save()
        {
            const string sp = "PKG_SCM_STR_RCV.SCM_GEN_ITEM_OB_INSERT";
            string jsonStr = "{";
            var ob = this;
            var i = 1;
            try
            {
                ...
                {
                     new CommandParameter() {ParameterName = "pSCM_GEN_ITEM_OB_ID", Value = ob.SCM_GEN_ITEM_OB_ID},
        ...
        public string FinalizeAll()
        {
            ...
            try
            {
                ...
                var ds = db.ExecuteStoredProcedure(new List<CommandParameter>()
                {                     
                     new CommandParameter() {ParameterName = "pXML_OB", Value = ob.XML_OB},                     
                     new CommandParameter() {ParameterName = "pCREATED_BY", Value = HttpContext.Current.Session["multiScUserId"]},
        ...
        {
            const string sp = "PKG_SCM_STR_RCV.SCM_GEN_ITEM_OB_";
            string jsonStr = "{";
            var ob = this;
            var i = 1;
            try
        ...
        {
            const string sp = "PKG_SCM_STR_RCV.SCM_GEN_ITEM_OB_DELETE";
            string jsonStr = "{";
            var ob = this;
            var i = 1;
            try
        ...
        public List<SCM_GEN_ITEM_OBModel> SelectAll(Int64 pageNumber, Int64 pageSize, Int64? pHR_COMPANY_ID = null, Int64? pSCM_STORE_ID = null, Int64? pINV_ITEM_CAT_ID = null)
        {
            string sp = "PKG_SCM_STR_RCV.SCM_GEN_ITEM_OB_SELECT";
            try
            {
                var obList = new List<SCM_GEN_ITEM_OBModel>();
                OraDatabase db = new OraDatabase();
                var ds = db.ExecuteStoredProcedure(new List<CommandParameter>()
                {
                ...
                {
                    SCM_GEN_ITEM_OBModel ob = new SCM_GEN_ITEM_OBModel();
                    ob.SCM_GEN_ITEM_OB_ID = (dr["SCM_GEN_ITEM_OB_ID"] == DBNull.Value) ? 0 : Convert.ToInt64(dr["SCM_GEN_ITEM_OB_ID"]);
                    ...
                    ob.ITEM_CAT_NAME_EN = (dr["MOU_CODE"] == DBNull.Value) ? string.Empty : Convert.ToString(dr["MOU_CODE"]);
        ...
        public SCM_GEN_ITEM_OBModel Select(long ID)
        {
            string sp = "PKG_SCM_STR_RCV.SCM_GEN_ITEM_OB_SELECT";
            try
            {
                var ob = new SCM_GEN_ITEM_OBModel();
                OraDatabase db = new OraDatabase();
                var ds = db.ExecuteStoredProcedure(new List<CommandParameter>()
                {
                     new CommandParameter() {ParameterName = "pSCM_GEN_ITEM_OB_ID", Value =0},
                     new CommandParameter() {ParameterName = "pOption", Value =3000},
                     new CommandParameter() {ParameterName = "pMsg", Value =500, Direction = ParameterDirection.Output}
                 }, sp);
                foreach (DataRow dr in ds.Tables[0].Rows)
